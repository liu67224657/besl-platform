...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td height="22" class="page_navigation_td">>> <a href="/msg/msglist" >手机消息推送</a> >> 修改待发送消息</td>
    </tr>
    <tr>
        <td height="100%" valign="top"><br>
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <td class="edit_table_header_td">&gt;修改待发送消息</td>
                </tr>
            </table>
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
            ...
            <table width="100%" border="0" cellspacing="1" cellpadding="0">
                <form action="/msg/editmsg" method="post">
                    <input type="hidden" name="pushMsgId" value="${pushMessage.pushMsgId}" >
                    <tr>
                        <td width="120" align="right" class="edit_table_defaulttitle_td">填写消息主体内容：</td>
                        <td nowrap class="edit_table_value_td">
                            <textarea rows="5" cols="100" name="msgBody" class="default_input_multiline" >${pushMessage.msgBody}</textarea>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td nowrap class="edit_table_value_td">
                            <select name="pushMessageType" class="default_select_single">
                                <c:forEach items="${pushMessageTypes}" var="pushMessageType">
                                    <option value="${pushMessageType.code}"
                                            <c:if test="${pushMessage.pushMessageType.code eq pushMessageType.code}">selected="selected"</c:if>><fmt:message key="def.pushmsg.type.${pushMessageType.code}.name" bundle="${def}"/></option>
                                </c:forEach>
                            </select>
                        </td>
                    </tr>
                        ...
                        <td nowrap class="edit_table_value_td">
                            <select name="pushMessageCode" class="default_select_single" onchange="change(this.value);">
                                <c:forEach items="${pushMsgCodes}" var="pushMsgCode">
                                    <option value="${pushMsgCode.code}"
                                        <c:if test="${pushMessage.pushMsgCode.code eq pushMsgCode.code}">selected="selected"</c:if>
                                            ><fmt:message key="def.pushmsg.code.${pushMsgCode.code}.name" bundle="${def}"/></option>
                                </c:forEach>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <div id="xxx">
                            <c:choose>
                                <c:when test="${pushMessage.pushMsgCode.code eq 'content'}">
                                    <div id="content">
                                        文章URL：<input type="text" name="url" class="default_input_singleline" value="${URL_WWW}/note/${cid}">
                                    </div>
                                ...
                                <c:otherwise>
                                    ...
                                    <div id="news" >
                                        分类角度：<input type="text" name="categoryAspect" class="default_input_singleline" value="${domain}"> <br/>
                                        分类编码：<input type="text" name="categoryCode" class="default_input_singleline" value="${cuno}">
                                    </div>
                                </c:otherwise>
                            </c:choose>
