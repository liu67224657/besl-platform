    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('.delete_link').click(function () {
                ...
                $.get('/gamev2/json/delete', {id: id}, function (req) {
                    if (req == 'success') {
                        alert('修改成功');
                        $('#tr_' + id).remove();
                    }else{
            ...
            $('#create_search').click(function(){
                $.get('/gamev2/json/search/indexbuild', function (req) {
                    if (req == 'success') {
                        alert('修改成功');
                    }else{
                        alert(req);
...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td height="22" class="page_navigation_td">>> 运营维护 >> Joymewiki管理>> 新版游戏资料库--游戏列表</td>
    </tr>
    <tr>
        <td height="100%" valign="top"><br>
            <table border="0" cellspacing="0" cellpadding="0">
                <tr>
                    ...
                    <td>
                        <form>
                        <input type="button"  class="default_button" value="创建搜索游戏的索引！慎点" id="create_search">
                        </form>
                    </td>
                </tr>
            </table>
                    ...
                    <c:when test="${list.size() > 0}">
                        <c:forEach items="${list}" var="dto" varStatus="st">
                            <tr id="tr_${dto.id}"
                                ...
                                <td nowrap class="target"><fmt:formatDate value='${dto.createTime}'
                                                                          pattern='yyyy-MM-dd HH:mm:ss'/></td>
                                <td nowrap class="target" title="${dto.extJson.recommend}">${dto.extJson.recommend}</td>
                                <td nowrap class="name"
                                    title="${dto.extJson.recommendAuth}">${dto.extJson.recommendAuth}</td>
                                ...
                                <td nowrap align="center">
                                    ...
                                    <a href="javascript:void(0);" class="delete_link" data-id="${dto.id}">删除</a>
                                </td>
                            </tr>
                        </c:forEach>
                        <tr>
