...
//                            $("#clientitem_" + returnid).removeClass(upclass);
        ...
        function create(lineId, itemType) {
            window.location.href = "/wechat/line/item/createpage?lineid=" + lineId + "&itemtype=" + itemType;
        }
    ...
    <tr>
        <td height="1" valign="top">
            <table>
                <tr>
                    <td height="1">
                        <form action="/wechat/line/item/list" method="post">
                            <table>
                                <tr>
                                    <td>
                                        按条件查询:
                    ...
                    <td height="1">
                        <form action="/wechat/line/item/createpage" method="post">
                            <table>
                                <tr>
                                    <td>
                                        <input type="hidden" name="lineid" value="${clientLine.lineId}"/>
                                        <input type="hidden" name="itemtype" value="${clientLine.itemType.code}"/>
                                        <p:privilege name="/wechat/line/item/createpage">
            ...
            <form action="/wechat/line/item/list" method="post">
                <table width="100%" border="0" cellspacing="1" cellpadding="0">
                    ...
                    <td nowrap align="left"><textarea rows="8" cols="40">${item.desc}</textarea></td>
                    <td nowrap align="left"><img src="${item.picUrl}" height="125" width="300"/></td>
                    <td nowrap align="left">${item.url}</td>
                    <td nowrap align="left"><c:choose><c:when
                            test="${item.param.isHot}">是</c:when><c:otherwise>否</c:otherwise></c:choose></td>
                    <td nowrap align="left"><c:choose><c:when
                            test="${item.param.isNew}">是</c:when><c:otherwise>否</c:otherwise></c:choose></td>
                    ...
                    <td nowrap align="left" <c:choose><c:when test="${item.validStatus.code == 'valid'}">style="color: #008000;"</c:when><c:otherwise>style="color: #ff0000;"</c:otherwise></c:choose>>
                        <fmt:message key="client.item.status.${item.validStatus.code}" bundle="${def}"/>
                    </td>
                    <td nowrap align="left">
                        <a href="/wechat/line/item/modifypage?itemid=${item.itemId}&lineid=${item.lineId}&itemtype=${item.itemType.code}">编辑</a>
                        <c:choose>
                            <c:when test="${item.validStatus.code == 'valid'}">
                                <a href="/wechat/line/item/delete?itemid=${item.itemId}&lineid=${item.lineId}">删除</a>
                            </c:when>
                            <c:otherwise>
                                <a href="/wechat/line/item/recover?itemid=${item.itemId}&lineid=${item.lineId}">恢复</a>
                            </c:otherwise>
                        </c:choose>
                    </td>
                    <td nowrap align="left">${item.param.startDate}-<br/>-${item.param.endDate}
                    </td>
                    <td nowrap align="left">
                    ...
                    <c:if test="${page.maxPage > 1}">
                        <tr class="list_table_opp_tr">
                            <td colspan="13">
                                <pg:pager url="/wechat/line/item/list"
