    ...
    <tr>
        <td height="100%" valign="top"><br>
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <td class="list_table_header_td">添加编辑人员</td>
                </tr>
            </table>
            <table width="100%" border="0" cellspacing="1" cellpadding="0">
                <form action="/viewline/setprivacy" method="POST">
                    <input name="categoryId" type="hidden" value="${categoryId}">
                    <tr>
                        <td height="1" colspan="4" class="default_line_td"></td>
                    </tr>
                    <tr>
                        <td width="120" align="right" class="edit_table_defaulttitle_td">请输入用户昵称：</td>
                        <td class="edit_table_value_td">
                            <input name="editor" type="text" class="default_input_singleline"
                                   size="24" maxlength="50" value="${editor}">
                            <c:if test="${errorMsgMap.containsKey('noteditor')}">
                                <fmt:message key="${errorMsgMap['noteditor']}" bundle="${error}"></fmt:message>
                            </c:if>
                            <c:if test="${errorMsgMap.containsKey('notprofile')}">
                                <fmt:message key="${errorMsgMap['notprofile']}" bundle="${error}"></fmt:message>
                            </c:if>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td nowrap class="edit_table_value_td">
                            <select name="privacy">
                                <c:forEach items="${privacyLevels}" var="privacyLevel">
                                    <option value="${privacyLevel.code}" >
