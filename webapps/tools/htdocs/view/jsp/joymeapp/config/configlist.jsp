...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $.post("/joymeapp/config/getversion", {
                appkey: appkey,
                platform: platform,
                enterprise: ent
            }, function (req) {
    ...
    <tr>
        <td height="100%" valign="top"><br>
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <td class="list_table_header_td">>app配置列表</td>
                </tr>
            </table>
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <table width="100%" border="0" cellspacing="1" cellpadding="0">
                ...
                <tr class="list_table_title_tr">
                    ...
                    <td nowrap align="center">审核期间广告</td>
                    <td nowrap align="center">摇一摇</td>
                    <td nowrap align="center">礼包中心</td>
                    <td nowrap align="center">上传BUCKET</td>
                ...
                <td nowrap>${config.info.defad_url}</td>
                <td nowrap><c:choose><c:when
                        test="${config.info.shake_open == 'true'}">开</c:when><c:otherwise>关</c:otherwise></c:choose>
                </td>
                <td nowrap><c:choose><c:when
                        test="${config.info.gift_open == 'true'}">开</c:when><c:otherwise>关</c:otherwise></c:choose></td>
                                <td nowrap>${config.bucket}</td>
                                <td nowrap>
                    <a href="/joymeapp/config/modifypage?configid=${config.configId}&appkey=${config.appKey}&platform=${config.platform.code}&version=${config.version}&channel=${config.channel}">编辑</a>&nbsp;
                    <p:privilege name="/joymeapp/config/delete">
                        <a href="/joymeapp/config/delete?configid=${config.configId}&appkey=${config.appKey}&platform=${config.platform.code}&version=${config.version}&channel=${config.channel}">删除</a>
                    </p:privilege>
                </td>
                <td nowrap>
                    <fmt:formatDate value="${config.createDate}"
                                    pattern="yyyy-MM-dd HH:mm:ss"/><br/>${config.createUserId}
                </td>
                <td nowrap>
