...
<tr>
    ...
    <td height="1">
        <textarea id="input_sortmsg" name="shortmessage" cols="40"
                  rows="5">${msgDTO.pushMessage.shortMessage}</textarea>
...
<tr>
    ...
    <td height="1" class="edit_table_defaulttitle_td">
        <select name="rtype" id="select_rtype">
            ...
            <option value="3"
                    <c:if test="${msgDTO.pushMessage.options.list.get(0).type == 3}">selected="selected"</c:if>>下载
            </option>
            <option value="4"
                    <c:if test="${msgDTO.pushMessage.options.list.get(0).type == 4}">selected="selected"</c:if>>菜单更新
            </option>
        </select>*<span style="color: red">必选项</span>
    </td>
    <td height="1">
...
<c:if test="${pushListType == 1}">
    <tr id="apppush">
        ...
        <td height="1">
            <select name="apppush" id="select_apppush" onchange="addPush()">
    ...
    <tr id="add_channel">
        ...
        <td height="1">
                <%--<input type="checkbox" id="all_channel" onclick="checkAllChannel($(this))"/>全选--%>
                <%--<input type="checkbox" id="inverse_channel" onclick="checkInverseChannel()"/>反选<br/>--%>
            <input type="checkbox" name="check_channel" value="" id="all_channel"/>全部<br/>
            <c:forEach var="channel" items="${channels}" varStatus="st">
                <input type="checkbox" name="check_channel" value="${channel.code}"/><fmt:message
                    key="joymeapp.channel.type.${channel.code}" bundle="${def}"/>&nbsp;
                <c:if test="${st.index % 6 ==5}"><br/></c:if>
            </c:forEach>
        </td>
        <td height="1">
