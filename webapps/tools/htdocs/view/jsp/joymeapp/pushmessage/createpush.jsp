...
$(document).ready(function () {
    ...
    $('#form_submit').bind('submit', function () {
        ...
        if (rtype == '4') {
            if (info.indexOf("http://www.joyme.com/appwiki/") < 0 && info.indexOf("http://t.cn") < 0) {
                alert("请填写正确的WIKI页地址：" + "\n" + "http://www.joyme.com/appwiki/ 或 http://t.cn/");
                return false;
            }
        }
        if (rtype == '9') {
...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td height="22" class="page_navigation_td">>> 运营维护 >> 着迷APP >> APP消息推送列表</td>
    </tr>
<tr>
<td height="100%" valign="top"><br>
    ...
    <form action="/joymeapp/push/create" method="post" id="form_submit">
        <table width="100%" border="0" cellspacing="1" cellpadding="0">
                    ...
                    <select name="platform" id="select_platform" onchange="getVersion()">
                        ...
                        <c:choose>
                            <c:when test="${pushListType ==0}">
                                <option value="0"
                                <c:if test="${0==platform}">selected="selected"</c:if>>IOS-appstore</option>
                                <option value="1"
                                <c:if test="${1==platform}">selected="selected"</c:if>>安卓</option>
                                <option value="2"
                                <c:if test="${2==platform}">selected="selected"</c:if>>IOS-企业版</option>
                            </c:when>
            ...
            <c:if test="${pushListType == 1}">
                <tr>
                    ...
                    <td height="1">
                        <img id="img_icon" src="/static/images/default.jpg" height="24" width="24"/>
            ...
            <tr>
                ...
                <td height="1">
                    <input id="msg_subject" type="text" name="subject" size="64" value="${subject}"/>
            ...
            <tr>
                ...
                <td height="1">
                    <textarea id="input_sortmsg" name="shortmessage" cols="40" rows="5">${shortMessage}</textarea>
            ...
            <c:if test="${pushListType == 0}">
                <tr>
                    ...
                    <td height="1">
                        <input id="input_badge" type="text" name="badge" value="0">*应用右上角的消息数目，通常为0或1
                    </td>
                    <td height="1">
                    </td>
                </tr>
                <tr>
                    ...
                    <td height="1">
                        <input type="radio" name="sound" value="default">系统默认&nbsp;
                        <input type="radio" name="sound" value="">无&nbsp;
                    </td>
                    <td height="1">
                    </td>
                </tr>
            ...
            <tr>
                ...
                <td height="1" class="edit_table_defaulttitle_td">
                    <select name="rtype" id="select_rtype">
                        <option value="0"
                        <c:if test="${0==redirectType}">selected="selected"</c:if>>打开应用</option>
                        <option value="1"
                        <c:if test="${1==redirectType}">selected="selected"</c:if>>文章页面</option>
                        <option value="2"
                        <c:if test="${2==redirectType}">selected="selected"</c:if>>wap页面</option>
                        <option value="3"
                        <c:if test="${3==redirectType}">selected="selected"</c:if>>下载</option>
                        <option value="4"
                        <c:if test="${4==redirectType}">selected="selected"</c:if>>菜单更新</option>
                    </select>*<span style="color: red">必选项</span>
                </td>
                <td height="1">
                </td>
            ...
            <tr>
                ...
                <td height="1">
                    <input id="input_info" type="text" name="info" size="64" value="${info}"/>
                    <a href="http://www.surl.sinaapp.com/" target="_blank">短链接工具</a><br/>
                    如果是纯文本，可以不填
                    <br/>cms文章单页,填写形如"syhbao/pingguoduan/201408/2549463.html"路径，客户端将域名"http://marticle.joyme.com/json/"写死在app内部
                    <br/>wap页、下载，填写完整的"http://"地址
                    <br/>菜单内容更新角标数目，填写形如"10076|3"，表示10076这个菜单下面有3个新内容，菜单ID和数目中间，用"|"竖线隔开。
                </td>
                <td height="1">
                </td>
            </tr>
            <c:if test="${pushListType == 0}">
                <tr>
                    ...
                    <td height="1" id="td_version">
                        ...
                        <span style="color: red">*请先选择应用、平台</span>
                    </td>
                    <td height="1">
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td height="1">
                        <input id="senddate" name="senddate"
                               onClick="WdatePicker({autoPickDate:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly/>
                    </td>
                    <td height="1">
                    </td>
                </tr>
