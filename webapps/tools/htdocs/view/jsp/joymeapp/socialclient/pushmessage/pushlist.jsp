...
<tr>
    <td height="22" class="page_navigation_td">>> 运营管理 >> 条目管理 >> 通知列表</td>
</tr>
<tr>
<td height="100%" valign="top"><br>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td class="list_table_header_td">>通知列表</td>
    </tr>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
        <td>
            <form action="/joymeapp/push/createpage" method="post">
                <input type="hidden" name="appid" value="${appId}"/>
                <input type="hidden" name="pushlisttype" value="${pushListType}">
                <c:choose>
...
<table width="100%" border="0" cellspacing="1" cellpadding="0">
    ...
    <td nowrap>
        <c:choose>
            <c:when test="${pushListType == 0}">
                <c:choose>
                    <c:when test="${msg.pushMessage.sendStatus.code == 'n'}">
                        <a href="/joymeapp/push/send?msgid=${msg.pushMessage.pushMsgId}&appid=${appId}&pushlisttype=${pushListType}"
                           name="send_click">推送</a>
                        <a href="/joymeapp/push/modifypage?msgid=${msg.pushMessage.pushMsgId}&appid=${appId}&pushlisttype=${pushListType}">编辑</a>
                        <a href="/joymeapp/push/remove?msgid=${msg.pushMessage.pushMsgId}&appid=${appId}&pushlisttype=${pushListType}">删除</a>
                    </c:when>
                    <c:when test="${msg.pushMessage.sendStatus.code == 'rj'}">
                        <a href="/joymeapp/push/detail?msgid=${msg.pushMessage.pushMsgId}&appid=${appId}&pushlisttype=${pushListType}">查看</a>
                    </c:when>
                    <c:when test="${msg.pushMessage.sendStatus.code == 'ing'}">
                        <a href="/joymeapp/push/detail?msgid=${msg.pushMessage.pushMsgId}&appid=${appId}&pushlisttype=${pushListType}">查看</a>
                    </c:when>
            ...
            <c:when test="${pushListType == 1}">
                <c:choose>
                    <c:when test="${msg.pushMessage.sendStatus.code == 'n'}">
                        <a href="/joymeapp/push/publish?msgid=${msg.pushMessage.pushMsgId}&appid=${appId}&pushlisttype=${pushListType}">发布</a>
                        <a href="/joymeapp/push/modifypage?msgid=${msg.pushMessage.pushMsgId}&appid=${appId}&pushlisttype=${pushListType}">编辑</a>
                        <a href="/joymeapp/push/remove?msgid=${msg.pushMessage.pushMsgId}&appid=${appId}&pushlisttype=${pushListType}">删除</a>
    ...
    <td>
        <fmt:formatDate value="${msg.pushMessage.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/><br/>${msg.pushMessage.createUserid}
    </td>
    </tr>
    </c:forEach>
    <tr>
