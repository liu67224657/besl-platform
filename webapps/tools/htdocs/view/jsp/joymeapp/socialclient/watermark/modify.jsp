...
//                    return false;
//                }
                var ios = $('#input_ios_pic').val();
                if (ios.length == 0) {
                    alert("请上传ios图片");
                    return false;
                }
//                var android = $('#input_android_pic').val();
//                if (android.length == 0) {
//                    alert("请上传android图片");
//                    return false;
//                }
        ...
        function check() {
            if ($("#select_bind_aid").find("option:selected").attr('data-status') == "y" && $("#select_bind_aid").find("option:selected").attr('value') != $('#input_bind_aid').val()) {
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td valign="top"><br>
            ...
            <form action="/joymeapp/social/watermark/modify" method="post" id="form_submit">
                <table width="100%" border="0" cellspacing="1" cellpadding="0">
                    <tr>
                        <td height="1" colspan="3" class="default_line_td">
                            <input type="hidden" name="wid" value="${watermark.watermarkId}"/>
                            ...
                            <input type="hidden" name="qbindaid" value="${queryBindAid}"/>
                    ...
                    <tr>
                        ...
                        <td height="1">
                            <input id="input_title" type="text" name="title" size="48" value="${watermark.title}"/>
                        </td>
                        <td height="1">
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td height="1">
                            <textarea id="input_description" type="text" name="description"
                                      style="height: 100px;width: 300px">${watermark.description}</textarea>
                        </td>
                        <td height="1">
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${fn:length(watermark.iosIcon) > 0}"><img id="ios_icon"
                                                                                        src="${watermark.iosIcon}"
                            ...
                            <input id="input_ios_icon" type="hidden" name="iosicon" value="${watermark.iosIcon}">*必填项(100px
                            * 100px)
                        </td>
                        <td height="1">
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${fn:length(watermark.androidIcon) > 0}"><img id="android_icon"
                                                                                            src="${watermark.androidIcon}"
                                                                                            class="img_pic"/></c:when>
                            ...
                            <span id="loading2" style="display:none" class="loading"><img
                                    src="/static/images/loading.gif"/></span>
                            <input id="input_android_icon" type="hidden" name="androidicon"
                                   value="${watermark.androidIcon}">
                        </td>
                        <td height="1">
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${fn:length(watermark.iosPic) > 0}"><img id="ios_pic"
                                                                                       src="${watermark.iosPic}"
                                                                                       class="img_pic"/></c:when>
                            ...
                            <span id="loading3" style="display:none" class="loading"><img
                                    src="/static/images/loading.gif"/></span>
                            <input id="input_ios_pic" type="hidden" name="iospic" value="${watermark.iosPic}">*必填项(640px
                            * 380px)
                        </td>
                        <td height="1">
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${fn:length(watermark.androidPic) > 0}"><img id="android_pic"
                                                                                           src="${watermark.androidPic}"
                                                                                           class="img_pic"/></c:when>
                            ...
                            <span id="loading4" style="display:none" class="loading"><img
                                    src="/static/images/loading.gif"/></span>
                            <input id="input_android_pic" type="hidden" name="androidpic"
                                   value="${watermark.androidPic}">
                        </td>
                        <td height="1">
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td height="1">
                            <c:forEach items="${SubscriptTypeColl}" var="subscriptType">
                                ...
                                <c:if test="${subscriptType.code == watermark.subscript.type}">checked="true"</c:if>
                            ...
                            <input type="text" class="default_input_singleline" size="16" maxlength="20"
                                   name="startdate" id="startdate" value="${watermark.subscript.startDate}"/>
                            &nbsp;--&nbsp;
                            <input type="text" class="default_input_singleline" size="16" maxlength="20" name="enddate"
                                   id="enddate" value="${watermark.subscript.endDate}"/>
                        </td>
                        <td height="1">
                        </td>
                    </tr>
                    <tr>
                        <td height="1" class="default_line_td" width="100">
                            绑定活动:
                            <input type="hidden" id="input_bind_aid" value="${watermark.activityId}"/>
                        </td>
                        <td height="1">
                            <select name="activityid" id="select_bind_aid" onchange="check()">
                                ...
                                <c:forEach items="${activityList}" var="activity">
                                    ...
                                    <c:choose>
                                        <c:when test="${activity.activityId != watermark.activityId && activity.bindStatus == 'y'}">style="color:
                                            #ff0000;"</c:when>
                                        <c:when test="${activity.activityId == watermark.activityId && activity.bindStatus == 'y'}">style="color:
                                            #008000;"</c:when>
                                    </c:choose>
                                    <c:if test="${activity.activityId == watermark.activityId}">selected="selected"</c:if>>
                                    ${activity.title}
                                    <c:choose>
                                        <c:when test="${activity.activityId != watermark.activityId && activity.bindStatus == 'y'}">(已有绑定)</c:when>
                                        <c:when test="${activity.activityId == watermark.activityId && activity.bindStatus == 'y'}">(当前绑定)</c:when>
                                    </c:choose>
                                    </option>
                                </c:forEach>
                            </select>
