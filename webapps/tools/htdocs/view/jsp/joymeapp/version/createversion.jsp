...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td height="100%" valign="top"><br>
            ...
            <form action="/joymeapp/version/create" method="post" id="form_submit">
                    ...
                    <tr>
                        ...
                        <td height="1">
                            <select name="enterprise" id="select_enterprise">
                                <option value="1" <c:if test="${enterprise == 1}">selected</c:if>>否</option>
                                <option value="2" <c:if test="${enterprise == 2}">selected</c:if>>是</option>
                            </select><span style="color: red">*必选项</span>
                        </td>
                        <td height="1">
                        </td>
                            ...
                            <select name="version" id="select_version">
                                ...
                                <c:forEach items="${versionList}" var="version">
                                    <option value="${version}"
                                            <c:if test="${version == appVersion}">selected="selected"</c:if>>${version}</option>
                                </c:forEach>
                            </select><span style="color: red">*必选项</span>
                        </td>
                        <td height="1" class=>
                            ...
                            <select name="channel" id="select_channel">
                                ...
                                <c:forEach var="chan" items="${channelList}">
                                    <option value="${chan.code}"
                                            <c:if test="${chan.code == channel}">selected="selected"</c:if>>
                                            ${chan.name}
                                    </option>
                                </c:forEach>
                            </select><span style="color: red">*必选项</span>
                    ...
                    <tr>
                        ...
                        <td height="1">
                            <textarea id="textarea_description" type="text" name="description"
                                      style="height: 100px;width: 400px">${description}</textarea>
                        </td>
                        <td height="1" class=>
                        </td>
                    </tr>
                            ...
                            <select name="versiontype" id="select_version_type">
                                <c:forEach items="${versionTypes}" var="vType">
                                    <option value="${vType.code}"
                                            <c:if test="${vType.code == versionType}">selected="selected"</c:if>>
