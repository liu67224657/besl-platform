...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td class="list_table_header_td">二级菜单编辑</td>
    </tr>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
...
<form action="/joymeapp/menu/submodify" method="get" id="form_submit" onsubmit="return isNotNull();">
...
<tr>
    ...
    <td height="1">
        <input id="input_text_menuName" type="text" name="menuname" size="80"
               value="${joymeAppMenu.menuName}"/>
    </td>
    <td height="1" class=>
    </td>
</tr>
<tr <c:if test="${displaytype==0}">style="display: none"</c:if>>
    <td height="1" class="default_line_td">
        菜单描述:
    </td>
    <td height="1">
        <textarea id="input_menuDesc" type="text" name="menudesc"
                  style="height: 100px;width: 600px">${joymeAppMenu.menuDesc}</textarea><span
...
<tr>
    ...
    <td height="1">
        <input id="input_text_url" type="text" name="url" size="80" value="${joymeAppMenu.url}"/>
    </td>
    <td height="1" class=>
    </td>
</tr>
...
<tr <c:if test="${displaytype == 0 || displaytype == 3}"> style="display: none"</c:if>>
    ...
    <td height="1">
        <img id="menu_pic" src="${joymeAppMenu.picUrl}"
        ...
        <input id="input_menu_pic" type="hidden" name="pic_url" value="${joymeAppMenu.picUrl}">
...
<tr>
    ...
    <td height="1" class="">
        <select name="displayType" id="displayType">
            <c:forEach var="dtype" items="${displayTypes}">
                <option value="${dtype.code}"
                        <c:if test="${dtype.code==joymeAppMenu.displayType.code}">selected="selected"</c:if>>
                    <fmt:message key="joymeapp.menu.displaytype.${dtype.code}" bundle="${def}"/>
                </option>
            </c:forEach>
        </select>
    </td>
        ...
        <select name="contenttype">
            ...
            <c:forEach items="${contentTypes}" var="ctype">
                <option value="${ctype.code}"
                        <c:if test="${ctype.code==joymeAppMenu.contentType.code}">selected</c:if>>
                    <fmt:message
                            key="app.menu.content.type.${ctype.code}" bundle="${def}"/></option>
            </c:forEach>
        </select>
    </td>
    <td height="1" class=>
...
<c:choose>
    <c:when test="${pMenu.moduleType.code ==1}">
        <tr>
            <td height="1" class="default_line_td">
                状态描述:
            </td>
            <td height="1" class="">
                <input type="text" name="statusdesc" id="input_statusdesc"
                       value="${joymeAppMenu.statusDesc}"/>*如：“即将开启/进行中...”
            </td>
            <td height="1" class=>
            </td>
        </tr>
    </c:when>
    <c:when test="${pMenu.moduleType.code ==3}">
        <tr>
            <td height="1" class="default_line_td">
                推荐星级:
            </td>
            <td height="1" class="">
                <input type="text" name="recomstar" id="input_recomstar"
                       value="${joymeAppMenu.recommendStar}"/>*1
                -- 5之间的数字
            </td>
            <td height="1" class=>
            </td>
...
<tr>
    ...
    <td height="1">
        <input id="lastmodifydate" name="lastmodifydate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly
               value="<fmt:formatDate value="${joymeAppMenu.lastModifyDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"></input>
    </td>
    <td height="1">
    </td>
</tr>
<c:choose>
    <c:when test="${joymeAppMenu.moduleType.code ==7}">
        <tr>
            <td height="1" class="default_line_td">
                内容类型:
            </td>
            <td height="1" class="">
                <input type="text" name="statusdesc" id="statusdesc" value="${joymeAppMenu.statusDesc}"/>
            </td>
            <td height="1" class=>
            </td>
        </tr>
