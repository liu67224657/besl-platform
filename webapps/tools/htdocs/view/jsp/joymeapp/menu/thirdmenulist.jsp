...
<head>
    ...
    <script language="JavaScript" type="text/JavaScript">
        function add(pid, appkey,displaytype,oid) {
            window.location.href = "/joymeapp/menu/thirdcreatepage?pid=" + pid + "&appkey=" + appkey+"&displaytype="+displaytype + "&oid=" + oid;
        }
    </script>
</head>
<body>
    ...
    <tr>
        <td height="100%" valign="top">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr class="toolbar_tr">
                    <td>
                        <p:privilege name="/joymeapp/menu/thirdcreatepage">
                            <input name="Submit" type="submit" class="default_button" value="添加三级级菜单"
                                   onClick="add('${pid}','${appkey}','${displaytype}','${oid}');">
                        </p:privilege>
                    </td>
                </tr>
            </table>
                        ...
                        <c:forEach items="${list}" var="submenu" varStatus="st">
                            <tr class="<c:choose><c:when test="${st.index % 2 == 0}">list_table_opp_tr </c:when><c:otherwise>list_table_even_tr </c:otherwise></c:choose>">
                                ...
                                <td nowrap><fmt:message key="joymeapp.menu.status.${submenu.removeStatus.getCode()}"
                                                        bundle="${def}"/></td>
                                <td nowrap>
                                    <a href="/joymeapp/menu/sort/up?menuId=${submenu.menuId}&appkey=${appkey}&pid=${pid}&oid=${oid}"><img
                                            src="/static/images/icon/up.gif"></a>
                                    <a href="/joymeapp/menu/sort/down?menuId=${submenu.menuId}&appkey=${appkey}&pid=${pid}&oid=${oid}"><img
                                            src="/static/images/icon/down.gif"></a>
                                </td>
                                <td nowrap>
                                    <a href="/joymeapp/menu/thirdmodifypage?menuid=${submenu.menuId}&appkey=${appkey}&pid=${pid}&oid=${oid}">编辑</a>
                                    <c:choose>
                                        <c:when test="${submenu.removeStatus.code!='n'}">
                                            <a href="/joymeapp/menu/thirdrecover?menuid=${submenu.menuId}&pid=${pid}&appkey=${appkey}&oid=${oid}">恢复</a>
                                        </c:when>
                                        <c:otherwise>
                                            <a href="/joymeapp/menu/thirddelete?menuid=${submenu.menuId}&pid=${pid}&appkey=${appkey}&oid=${oid}">删除</a>
                                        </c:otherwise>
                                    </c:choose></td>
