...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td height="22" class="page_navigation_td">>> 运营维护 >> 大动漫管理 >> 大动漫分享列表</td>
    </tr>
<tr>
<td height="100%" valign="top"><br>
    <table border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td class="list_table_header_td">大动漫分享列表</td>
        </tr>
    </table>
        ...
        <tr>
            <td>
                    <table width="100%">
                        <tr>
                            <td>
                                <form action="/joymeapp/anime/share/list" method="post">
                                    <select name="appkey">
            ...
            <td>
                <table width="100%">
                    <tr>
                        <td>
                            <form action="/joymeapp/anime/share/createpage" method="post">
                                <select name="appkey">
                                    <option value="0G30ZtEkZ4vFBhAfN7Bx4v" <c:if test="${appkey=='0G30ZtEkZ4vFBhAfN7Bx4v'}">selected</c:if>>海贼迷</option>
                                    <option value="1zBwYvQpt3AE6JsykiA2es" <c:if test="${appkey=='1zBwYvQpt3AE6JsykiA2es'}">selected</c:if>>火影迷</option>
                                </select>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="0">
        ...
        <tr class="list_table_title_tr">
            ...
            <td nowrap align="center" width="80">专题ID</td>
            ...
            <c:when test="${list.size() > 0}">
                <c:forEach items="${list}" var="dto" varStatus="st">
                    ...
                    <td nowrap align="center">
                        <a href="/joymeapp/anime/share/modifypage?shareid=${dto.share_id}&pager.offset=${page.startRowIdx}&appkey=${appkey}">编辑</a>

                        <c:choose>
                            <c:when test="${dto.remove_status.getCode()=='n'}">
                                <a href="/joymeapp/anime/share/modify?appkey=${dto.appkey}&platform=${dto.platform}&shareid=${dto.share_id}&del=y&pager.offset=${page.startRowIdx}&share_type=${dto.share_type.code}&activityid=${dto.activityid}&appkey=${appkey}">删除</a>
                            </c:when>
                            <c:otherwise>
                                <a href="/joymeapp/anime/share/modify?appkey=${dto.appkey}&platform=${dto.platform}&shareid=${dto.share_id}&del=n&pager.offset=${page.startRowIdx}&share_type=${dto.share_type.code}&activityid=${dto.activityid}&appkey=${appkey}">恢复</a>
                            </c:otherwise>
                        </c:choose>
        ...
        <c:if test="${page.maxPage > 1}">
            <tr class="list_table_opp_tr">
                <td colspan="10">
                    <pg:pager url="/joymeapp/anime/virtual/list"
