    ...
    <tr>
        <td height="100%" valign="top"><br>
            ...
            <table>
                <tr>
                    <td>
                        <%--<form action="/joymeapp/wiki/top/list" method="post">--%>
                            <%--<table width="100%">--%>
                                <%--<tr>--%>
                ...
                <c:choose>
                    <c:when test="${list.size() > 0}">
                        <c:forEach items="${list}" var="dto" varStatus="st">
                            <tr id="clientitem_${dto.lineId}"
                                class="<c:choose><c:when test="${st.index % 2 == 0}">list_table_opp_tr</c:when><c:otherwise>list_table_even_tr</c:otherwise></c:choose>">
                                <td nowrap>${dto.lineId}</td>
                                <td nowrap><a href="/wiki/top/item/list?linecode=${dto.code}">${dto.lineName}</a>
                                </td>
                                <td nowrap>${dto.code}</td>
                                <td nowrap>
                                    <c:choose>
                                ...
                                <td nowrap>
                                        <%--<a href="/joymeapp/wiki/top/modifypage?lineid=${dto.lineId}">编辑</a>&nbsp;--%>
                                    <c:choose>
                                        <c:when test="${dto.validStatus.code eq 'valid'}">
                                            <a href="/joymeapp/wiki/top/delete?lineid=${dto.lineId}">删除</a>
                                        </c:when>
                                        <c:otherwise>
                                            <a href="/joymeapp/wiki/top/recover?lineid=${dto.lineId}">恢复</a>
                                        </c:otherwise>
                                    </c:choose>
