...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td height="22" class="page_navigation_td">>> 运营维护 >> 视频管理 >>游戏管理</td>
    </tr>
    <tr>
        <td height="100%" valign="top"><br>
            <table border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <td class="list_table_header_td">着迷视频游戏管理</td>
                </tr>
            </table>
            <table border="0" cellspacing="0" cellpadding="0">
                <tr>
                    ...
                    <td>
                        <form method="post" action="/video/game/list">
                            <input type="text" name="appname" value="${appName}"/>
                            <input type="submit" name="button" value="查询"/>
                        </form>
                    </td>
                    <td class="list_table_header_td">
                        <form method="post" action="/video/game/createpage">
                            <input type="submit" name="button" value="添加游戏"/>
                        </form>
                    </td>
                </tr>
            </table>
                    ...
                    <c:when test="${list.size() > 0}">
                        <c:forEach items="${list}" var="app" varStatus="st">
                            ...
                            ${st.index % 2 == 0}">list_table_opp_tr</c:when><c:otherwise>list_table_even_tr</c:otherwise></c:choose>">
                                ...
                                <td nowrap>
                                    <a href="/video/game/modifypage?appid=${app.appId}">编辑</a>

                                    <%--<c:choose>--%>
                                        <%--<c:when test="${app.validStatus.code!='valid'}">--%>
                                            <%--<a href="/video/game/delete?appid=${app.appId}&status=valid">恢复</a>--%>
                                        <%--</c:when>--%>
                                        <%--<c:otherwise>--%>
                                            <%--<a href="/video/game/delete?appid=${app.appId}&status=invalid">删除</a>--%>
                                        <%--</c:otherwise>--%>
                                    <%--</c:choose>--%>
