...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td height="22" class="page_navigation_td">>> 运营维护 >> 玩霸-虚拟用户 >> 问答通知</td>
    </tr>
    <tr>
        <td height="100%" valign="top"><br>
            <table border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <td class="list_table_header_td">问答通知</td>
                </tr>
            </table>
            ...
            <table width="100%" border="0" cellspacing="1" cellpadding="0">
                ...
                <c:choose>
                    <c:when test="${list.size() > 0}">
                        <c:forEach items="${list}" var="userNotice" varStatus="st">
                            <tr id="tr_${userNotice.userNoticeId}"
                                class="<c:choose><c:when test="${st.index % 2 == 0}">list_table_opp_tr</c:when><c:otherwise>list_table_even_tr</c:otherwise></c:choose>">
                                <td nowrap>${userNotice.userNoticeId}</td>
                                <td nowrap>${userNotice.body}</td>
                                <td nowrap>
                                    <fmt:formatDate value="${userNotice.createTime}"
                                                    pattern="yyyy-MM-dd HH:mm:ss"/>
                               </td>
                                <td nowrap>
                                    &nbsp;&nbsp;&nbsp;&nbsp;
                                    <a href="/wanba/ask/answer/list?qid=${userNotice.destId}" target= "_blank">进入详情页</a>
                                </td>
                ...
                <c:if test="${page.maxPage > 1}">
                    <tr class="list_table_opp_tr">
                        <td colspan="9">
                            <pg:pager url="/wanba/virtual/questionlist"
