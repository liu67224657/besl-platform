        ...
        $(document).ready(function () {
            $('#Submit').bind('click', function () {
                ...
                $.ajax({
                    ...
                    data: {title: title,text:text,askpid:askpid,timelimit:timelimit,tagid:tagid,timeid:${release.timeid},releasetime:releasetime},
                    dataType: "json",
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <table width="100%" border="0" cellspacing="1" cellpadding="0">
                        ...
                        <td height="1">
                            <select name="askpid" id="askpid">
                                <c:forEach items="${list}" var="profile" varStatus="st">
                                    <option value="${profile.profileid}"  <c:if test="${questionRelease.askpid==profile.profileid}">selected</c:if> ><c:if test="${profileMap[profile.profileid]!=null}">${profileMap[profile.profileid].nick}</c:if></option>
                                </c:forEach>
                            </select>
                            <span></span>
                        </td>
                        ...
                        <td height="1">
                            <select name="tagid" id="tagid">
                                <c:forEach items="${animeTagList}" var="tag" varStatus="st">
                                    <option value="${tag.tag_id}" <c:if test="${questionRelease.tagid==tag.tag_id}">selected</c:if>>${tag.tag_name}</option>
                                </c:forEach>
                            </select>
                            <span></span>
                        </td>
                        ...
                        <td height="1">
                            <select name="timelimit" id="timelimit">
                                <c:forEach items="${questionconfigtype}" var="questionconfig" varStatus="st">
                                    <option value="${questionconfig.timeLimit}" <c:if test="${questionRelease.timelimit==questionconfig.timeLimit}">selected</c:if>>${questionconfig.timeLimit/(1000*60)}分钟__${questionconfig.questionPoint}积分</option>
                                </c:forEach>
                            </select>
                            <span></span>
                        </td>
                    ...
                    <tr>
                        ...
                        <td height="1">
                            <input name="title" id="title" type="text" size="100" class="default_button" value="${questionRelease.title}">
                        <td height="1" class=>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td height="1">
                            <input type="text" name="releasetime" id="releasetime"
                                   ...
                                   value="<fmt:formatDate value="${release.releaseTime }" pattern="yyyy-MM-dd HH:mm:ss"/>">
                        </td>
                        <td height="1">
                        </td>
                    </tr>
...
<script>
    ...
    ue.ready(function() {
        //设置编辑器的内容
        ue.setContent("${questionRelease.text}");
