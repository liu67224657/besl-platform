...
<tr>
    ...
    <td height="1" class="edit_table_defaulttitle_td">
        <select name="rtype" id="select_rtype" disabled="disabled">
            <option value="-2"
                    <c:if test="${msgDTO.pushMessage.options.list.get(0).type == -2}">selected="selected"</c:if>>wap页面
            </option>
            <option value="-4"
            ...
            <option value="50"
                    <c:if test="${msgDTO.pushMessage.options.list.get(0).type == 50}">selected="selected"</c:if>>
                个人主页
            </option>
            <option value="51"
                    <c:if test="${msgDTO.pushMessage.options.list.get(0).type == 51}">selected="selected"</c:if>>活动列表
            </option>
            <option value="52"
                    <c:if test="${msgDTO.pushMessage.options.list.get(0).type == 52}">selected="selected"</c:if>>活动页
            </option>
            <option value="53"
                    <c:if test="${msgDTO.pushMessage.options.list.get(0).type == 53}">selected="selected"</c:if>>问答详情页
            </option>
        </select>
    </td>
    <td height="1">
...
<tr>
    ...
    <td height="1">
        <input id="input_info" type="text" name="info" size="64"
               value="${msgDTO.pushMessage.options.list.get(0).info}" disabled="disabled"/>
...
<c:if test="${pushListType == 0 || pushListType == 3 || pushListType == 4}">
            ...
            <c:forEach var="version" items="${versionList}" varStatus="st">
                <input type="checkbox" name="check_version" value="${version}"
                        <c:forEach var="hasver" items="${versionSet}">
                            <c:if test="${hasver == version}">checked="checked"</c:if></c:forEach> disabled="disabled"/>${version}&nbsp;
    ...
    <c:if test="${fn:length(msgDTO.pushMessage.tags) > 0}">
            <tr>
                ...
                <td height="1">
                    <textarea id="textarea_tags" name="tags" cols="60" rows="10" disabled="disabled">${msgDTO.pushMessage.tags}</textarea>
                    *礼包上架的自生成标签，暂不可更改。
                </td>
                <td height="1">
                </td>
