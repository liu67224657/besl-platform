...
function getVersion() {
    ...
    if (pushListType == 0 || pushListType == 3 || pushListType == 4) {
        ...
        $.post("/joymeapp/push/getversion", {platform: platform, appid: appId, pushlisttype: 5, enterprise:ent}, function (req) {
            var resMsg = eval('(' + req + ')');
            if (resMsg.rs == 0) {
                return false;
            } else {
...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td height="22" class="page_navigation_td">>> 运营维护 >> 玩霸-问答 >> 玩霸-问答消息中心列表</td>
    </tr>
<tr>
...
<table width="30%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td class="list_table_header_td"><c:if test="${pushListType == 0 || pushListType == 3 || pushListType==4}">创建推送消息</c:if><c:if
                test="${pushListType == 5}">创建系统通知</c:if></td>
        <td class="">
            <form></form>
        </td>
    </tr>
            ...
            <td height="1" class="edit_table_defaulttitle_td">
                <select name="rtype" id="select_rtype">
                    ...
                    <c:forEach items="${WanbaJt}" var="type">
                        <option value="${type.code}"  <c:if test='${type.code==redirectType}'>selected</c:if> >${type.name}</option>
                    </c:forEach>
                </select>
            </td>
            <td height="1">
