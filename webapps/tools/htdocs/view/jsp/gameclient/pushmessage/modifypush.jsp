...
<table width="30%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td class="list_table_header_td"><c:if
                test="${pushListType == 0 || pushListType == 3 || pushListType == 4}">创建推送消息</c:if><c:if
        ...
        <tr>
            ...
            <td height="1">
                <select id="select_enterprise" name="enterprise" onchange="getVersion()">
                    <option value="1" <c:if test="${msgDTO.pushMessage.enterpriseType.code == 1}">selected="selected"</c:if>>否</option>
                    <option value="2" <c:if test="${msgDTO.pushMessage.enterpriseType.code == 2}">selected="selected"</c:if>>是</option>
                </select>
            </td>
        </tr>
    </c:if>
...
<tr>
    ...
    <td height="1" class="edit_table_defaulttitle_td">
        <select name="rtype" id="select_rtype">
            <option value="-1"
                    <c:if test="${msgDTO.pushMessage.options.list.get(0).type == -1}">selected="selected"</c:if>>打开应用
            </option>
            <option value="-2"
            ...
            <option value="23"
                    <c:if test="${msgDTO.pushMessage.options.list.get(0).type == 23}">selected="selected"</c:if>>
                wap页(无头部导航)
            </option>
            <option value="24"
                    <c:if test="${msgDTO.pushMessage.options.list.get(0).type == 24}">selected="selected"</c:if>>游戏详情页
            </option>
            <option value="-4"
                    <c:if test="${msgDTO.pushMessage.options.list.get(0).type == -4}">selected="selected"</c:if>>下载
            </option>
        </select><span style="color: red">*必选项</span>
    </td>
    <td height="1">
...
<tr>
    ...
    <td height="1">
        <input id="input_info" type="text" name="info" size="64"
               value="${msgDTO.pushMessage.options.list.get(0).info}"/><a href="http://www.surl.sinaapp.com/"
...
<c:if test="${pushListType == 0 || pushListType ==3 || pushListType==4}">
    ...
    <c:if test="${fn:length(msgDTO.pushMessage.tags) > 0}">
        <tr>
            <td height="1" class="default_line_td">
                标签：
            </td>
            <td height="1">
                <textarea id="textarea_tags" name="tags" cols="60" rows="10"
                          readonly="readonly">${msgDTO.pushMessage.tags}</textarea>
                *礼包上架的自生成标签，暂不可更改。
            </td>
            <td height="1">
            </td>
    ...
    <tr>
        ...
        <td height="1">
            <input id="senddate" name="senddate" value="<fmt:formatDate value="${msgDTO.pushMessage.sendDate}"
            pattern="yyyy-MM-dd HH:mm:ss"/>" onClick="WdatePicker({autoPickDate:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})"
            readonly/>
        </td>
        <td height="1">
        </td>
