...
function getVersion() {
    ...
    if (pushListType == 0 || pushListType == 3 || pushListType == 4) {
        ...
        $.post("/joymeapp/push/getversion", {platform: platform, appid: appId, pushlisttype: 0, enterprise:ent}, function (req) {
            var resMsg = eval('(' + req + ')');
            if (resMsg.rs == 0) {
                return false;
            } else {
...
<table width="30%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td class="list_table_header_td"><c:if test="${pushListType == 0 || pushListType == 3 || pushListType==4}">创建推送消息</c:if><c:if
...
<form action="/joymeapp/push/create" method="post" id="form_submit">
    <table width="100%" border="0" cellspacing="1" cellpadding="0">
        ...
        <tr>
            ...
            <td height="1">
                <select name="platform" id="select_platform" onchange="getVersion()">
                    ...
                    <option value="0"
                    <c:if test="${0==platform}">selected="selected"</c:if>>IOS</option>
                    <option value="1"
        ...
        <c:if test="${pushListType == 0 || pushListType == 3 || pushListType==4}">
            <tr>
                <td height="1" class="default_line_td">
                    是否企业版
                </td>
        ...
        <tr>
            ...
            <td height="1">
                <textarea id="input_sortmsg" name="shortmessage" cols="40" rows="5">${shortMessage}</textarea><span
        ...
        <tr>
            ...
            <td height="1" class="edit_table_defaulttitle_td">
                <select name="rtype" id="select_rtype">
                    <option value="-1"
                    <c:if test="${-1==redirectType}">selected="selected"</c:if>>打开应用</option>
                    <option value="-2"
                    <c:if test="${-2==redirectType}">selected="selected"</c:if>>wap页(有头部导航)</option>
                    <option value="23"
                    <c:if test="${23==redirectType}">selected="selected"</c:if>>wap页(无头部导航)</option>
                    <option value="24"
                    <c:if test="${24==redirectType}">selected="selected"</c:if>>游戏详情页</option>
                    <option value="-4"
                    <c:if test="${-4==redirectType}">selected="selected"</c:if>>下载</option>
                </select><span style="color: red">*必选项</span>
            </td>
            <td height="1">
            </td>
        ...
        <tr>
            ...
            <td height="1">
                ...
                如果是打开应用，可以不填。游戏详情页填写游戏资料库ID。wap页、下载需要填写完整的http://链接。
            </td>
            <td height="1">
            </td>
        </tr>
        <c:if test="${pushListType == 0 || pushListType == 3 || pushListType == 4}">
            <tr>
                ...
                <td height="1" id="td_version">
                    ...
                    <span style="color: red">*请先选择平台</span>
                </td>
                <td height="1">
                </td>
            </tr>
            ...
            <c:if test="${fn:length(tags) > 0}">
                <tr>
                    ...
                    <td height="1">
                        <textarea id="textarea_tags" name="tags" cols="60" rows="10"
                                  readonly="readonly">${tags}</textarea>
                        *礼包上架的自生成标签，暂不可更改。
                    </td>
                    <td height="1">
                    </td>
