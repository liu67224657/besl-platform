...
<table width="80%" border="0" cellspacing="0" cellpadding="0">
    <form action="/gameclient/advertise/list" method="post" id="form_submit_search">
        <tr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td>
            <form method="post" action="/gameclient/advertise/createpage">
                <table>
                    <tr>
                        <td>
...
<table width="100%" border="0" cellspacing="1" cellpadding="0"style="vertical-align:middle; text-align:center;">
    ...
    <tr class="list_table_title_tr">
        ...
        <td nowrap align="center" width="200">平台</td>
            ...
            <c:forEach items="${list}" var="ad" varStatus="st">
                <tr class="<c:choose><c:when test="${st.index % 2 == 0}">list_table_opp_tr</c:when><c:otherwise>list_table_even_tr</c:otherwise></c:choose>">
                    ...
                    <td nowrap><c:if test="${ad.appPlatform.code == '0'}">IOS</c:if>
                        <c:if test="${ad.appPlatform.code == '1'}">Android</c:if></td>
                    ...
                    <td nowrap>
                         <a href="/gameclient/advertise/modifypage?advertiseId=${ad.advertiseId}&pager.offset=${page.startRowIdx}">编辑</a>
                        <c:choose>
                            <c:when test="${ad.removeStatus.code eq 'n'}">
                                    <a href="/gameclient/advertise/modify?advertiseId=${ad.advertiseId}&removestatus=y&pager.offset=${page.startRowIdx}">停用</a>
                            </c:when>
                            <c:when test="${ad.removeStatus.code eq 'y'}">
                                 <a href="/gameclient/advertise/modify?advertiseId=${ad.advertiseId}&removestatus=n&pager.offset=${page.startRowIdx}">启用</a>
                            </c:when>
                        </c:choose>
                        &nbsp;
                    </td>
    ...
    <c:if test="${page.maxPage > 1}">
        <tr class="list_table_opp_tr">
            <td colspan="9">
                ...
                <LABEL>
                    <pg:pager url="/gameclient/advertise/list"
