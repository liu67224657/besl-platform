...
dhtmlXTreeObject.prototype._globalIdStorageFind = function(itemId, skipXMLSearch, skipParsing, isreparse) {
    ...
    if (z) {
        ...
        if ((isreparse) && (this._edsbpsA)) {
            for (var j = 0; j < this._edsbpsA.length; j++)if (this._edsbpsA[j][2] == itemId) {
...
dhtmlXTreeObject.prototype.reParse = function(node) {
    ...
    for (var i = 0; i < this.parsedArray.length; i++)node.htmlNode.childNodes[0].appendChild(this.parsedArray[i]);
    ...
    if (this._srnd && !node._sready)this.prepareSR(node.id);
    return true
};
dhtmlXTreeObject.prototype.preParse = function(itemId) {
    ...
    if (result) {
        ...
        if (!n)dhtmlxError.throwError("getItem", "The item " + itemId + " not operable. Seems you have non-unique|incorrect IDs in tree's XML.", itemId)
    }
    ;
    return n
};
...
dhtmlXTreeObject.prototype._parseItem = function(c, temp, preNode, befNode) {
    ...
    if (a.tooltip)if (this._dhxTT)dhtmlxTooltip.setTooltip(newNode.span.parentNode.parentNode, a.tooltip); else
...
dhtmlXTreeObject.prototype._parse = function(p, parentId, level, start) {
    ...
    this.parsCount--;
    var that = this;
    if (this._edsbps)window.setTimeout(function() {
...
dhtmlXTreeObject.prototype._selectItem = function(node, e) {
    ...
    if ((!node.i_sel) && ((!this._amselS) || (this._selected.length == 0) || (this._selected[0].parentObject == node.parentObject)))
        if ((this._amsel) && (e) && (e.shiftKey) && (this._selected.length != 0) && (this._selected[this._selected.length - 1].parentObject == node.parentObject)) {
            ...
            ;
            for (var i = a; i <= b; i++)if (!node.parentObject.childNodes[i].i_sel)this._markItem(node.parentObject.childNodes[i])
        } else
...
dhtmlXTreeObject.prototype._createItem = function(acheck, itemObject, mode) {
    ...
    if (this.ettip)if (this._dhxTT)dhtmlxTooltip.setTooltip(tr, itemObject.label); else
...
dhtmlXTreeObject.prototype._fixChildCountLabel = function(itemNode, index) {
    ...
    switch (this.childCalc) {
        case 1:
            if (itemNode.childsCount != 0)itemNode.span.innerHTML = itemNode.label + this.htmlcA + itemNode.childsCount + this.htmlcB; else itemNode.span.innerHTML = itemNode.label;
            break;
        case 2:
            var z = this._getLeafCount(itemNode);
            if (z != 0)itemNode.span.innerHTML = itemNode.label + this.htmlcA + z + this.htmlcB; else itemNode.span.innerHTML = itemNode.label;
            break;
        case 3:
...
dhtmlXTreeObject.prototype.setUserData = function(itemId, name, value) {
    ...
    if (!sNode)return;
    if (name == "hint")if (this._dhxTT)dhtmlxTooltip.setTooltip(sNode.htmlNode.childNodes[0].childNodes[0], value); else
...
dhtmlXTreeObject.prototype.setItemText = function(itemId, newLabel, newTooltip) {
    ...
    if (this._dhxTT)dhtmlxTooltip.setTooltip(temp.span.parentNode.parentNode, (newTooltip || "")); else
...
dhtmlXTreeObject.prototype.getSubItems = function(itemId) {
    ...
    if (!temp)return 0;
    if (temp.unParsed)return (this._getSubItemsXML(temp.unParsed));
    var z = "";
...
dhtmlXTreeObject.prototype._getAllSubItems = function(itemId, z, node) {
    ...
    ;
    if (temp.unParsed)z = this._getAllSubItemsXML(itemId, z, temp.unParsed);
    return z
};
...
dhtmlXTreeObject.prototype._setSubChecked = function(state, sNode) {
    ...
    if (sNode.unParsed)this._setSubCheckedXML(state, sNode.unParsed)
    ...
    if (htmlNode.parentObject.span) {
        ...
        if (this.dadmode == 2) {
            ...
            if ((Math.abs(z) - htmlNode.offsetHeight / 6) > 0) {
                this.dadmodec = 1;
                if (z < 0)this.dadmodefix = 0 - htmlNode.offsetHeight
...
dhtmlXTreeObject.prototype._lastChild = function(item) {
    if (item.childsCount)return this._lastChild(item.childNodes[item.childsCount - 1]); else return item
};
dhtmlXTreeObject.prototype._getPrevNode = function(node, mode) {
    ...
    if (node.parentObject)return node.parentObject; else return -1
};
...
dhtmlXTreeObject.prototype._checkMSelectionLogic = function() {
    var usl = new Array();
    for (var i = 0; i < this._selected.length; i++)for (var j = 0; j < this._selected.length; j++)if ((i != j) && (this._checkPNodes(this._selected[j], this._selected[i])))
        usl[usl.length] = this._selected[j];
    for (var i = 0; i < usl.length; i++)this._unselectItem(usl[i])
};
...
dhtmlXTreeObject.prototype.getItemParsingState = function(itemId) {
    ...
    if (!z)return 0;
    if (this._edsbpsA)for (var i = 0; i < this._edsbpsA.length; i++)if (this._edsbpsA[i][2] == itemId)return -1;
    return 1
};
...
dhtmlXTreeObject.prototype._distributedStep = function(pId) {
    ...
    if ((!this._edsbpsA.length)) {
        window.setTimeout(function() {
            if (self.onXLE)self.onXLE(self, pId);
...
dhtmlXTreeObject.prototype.smartRefreshItem = function(itemId, source) {
    var sNode = this._globalIdStorageFind(itemId);
    for (var i = 0; i < sNode.childsCount; i++)sNode.childNodes[i]._dmark = true;
    this.waitUpdateXML = true;
    if (source && source.exists)this._parse(source, itemId); else
...
dhtmlXTreeObject.prototype.refreshItems = function(itemIdList, source) {
    ...
    for (var i = 0; i < z.length; i++)this.waitUpdateXML[z[i]] = true;
...
dhtmlXTreeObject.prototype.setItemTopOffset = function(itemId, value) {
    ...
    for (var i = 0; i < z.childNodes.length; i++) {
        if (i != 0)z.childNodes[i].style.height = 18 + parseInt(value) + "px"; else {
...
dhtmlXTreeObject.prototype.enableRadioButtons = function(itemId, mode) {
    ...
    if ((mode) && (!node._r_logic)) {
        node._r_logic = true;
        for (var i = 0; i < node.childsCount; i++)this._setCheck(node.childNodes[i], node.childNodes[i].checkstate)
    }
    ;
    if ((!mode) && (node._r_logic)) {
        node._r_logic = false;
...
dhtmlXTreeObject.prototype.enableMultiLineItems = function(width) {
    if (width === true)this.mlitems = "100%"; else this.mlitems = width
};
...
dhtmlXTreeObject.prototype.clearSelection = function(itemId) {
    if (itemId)this._unselectItem(this._globalIdStorageFind(itemId)); else
        this._unselectItems()
};
...
dhtmlXTreeObject.prototype._getAllChecked = function(htmlNode, list, mode) {
    ...
    ;
    if (htmlNode.unParsed)list = this._getAllCheckedXML(htmlNode.unParsed, list, mode);
    if (list)return list; else return ""
};
