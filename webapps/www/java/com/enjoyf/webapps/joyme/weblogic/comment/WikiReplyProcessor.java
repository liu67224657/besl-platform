 ...
 */
@Component
public class WikiReplyProcessor extends AbstractCommentProcessor {
    ...
    public ResultObjectMsg buildResultObjectMsg(ResultObjectMsg resultObjectMsg, String uniKey, CommentDomain commentDomain, String jsonParam, int pageNum, int pageSize, Long authorUid, boolean desc) throws ServiceException {
        ...
        ResultBean resultBean = getOrCreateCommentBean(commentId, uniKey, commentDomain, jsonParam);
        if (resultBean.getResult().getCode() != ResultCodeConstants.SUCCESS.getCode()) {
            resultObjectMsg.setRs(resultBean.getResult().getCode());
            resultObjectMsg.setMsg(resultBean.getResult().getMsg());
            return resultObjectMsg;
        }

        CommentBean commentBean = resultBean.getBean();

        PageRows<MainReplyDTO> mainReplyRows = commentWebLogic.queryMainReplyDTO(commentBean, pageNum, pageSize, desc);

        return buildResultMsg(resultObjectMsg, commentBean, mainReplyRows, authorUid);
    }

    @Override
    public ResultObjectMsg buildResultObjectMsgByReplyId(ResultObjectMsg resultObjectMsg, String uniKey, CommentDomain commentDomain, String jsonParam, long replyId, int pageSize, Long authorUid, boolean desc) throws ServiceException {
        String commentId = CommentUtil.genCommentId(uniKey, commentDomain);
        ResultBean resultBean = getOrCreateCommentBean(commentId, uniKey, commentDomain, jsonParam);
        if (resultBean.getResult().getCode() != ResultCodeConstants.SUCCESS.getCode()) {
            resultObjectMsg.setRs(resultBean.getResult().getCode());
        ...
        PageRows<MainReplyDTO> mainReplyRows = commentWebLogic.queryMainReplyDTOByQueryReplyId(commentBean, replyId, pageSize, desc);

        return buildResultMsg(resultObjectMsg, commentBean, mainReplyRows, authorUid);
    }

    private ResultObjectMsg buildResultMsg(ResultObjectMsg resultObjectMsg, CommentBean commentBean, PageRows<MainReplyDTO> mainReplyRows, Long authorUid) throws ServiceException {
    ...
    private ResultBean getOrCreateCommentBean(String commentId, String uniKey, CommentDomain commentDomain, String jsonParam) throws ServiceException {
        CommentBean commentBean = CommentServiceSngl.get().getCommentBeanById(commentId);
        if (commentBean == null) {
            if (StringUtil.isEmpty(jsonParam)) {
                return new ResultBean(ResultCodeConstants.COMMENT_PARAM_JSONPARAM_NULL, null);
            }
            CommentJsonParam param = null;
            try {
                param = CommentJsonParam.parse(jsonParam);
            } catch (Exception e) {
                return new ResultBean(ResultCodeConstants.COMMENT_PARAM_COMMENTPARAM_ERROR, null);
            }
            commentBean = commentWebLogic.createCommentBean(uniKey, commentDomain, param);

            return new ResultBean(ResultCodeConstants.SUCCESS, commentBean);
        }
