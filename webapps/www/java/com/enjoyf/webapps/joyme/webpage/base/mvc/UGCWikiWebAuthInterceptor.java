 ...
 * Description:
 */
public class UGCWikiWebAuthInterceptor extends AbstractUGCWikiAuthInterceptor {
    private Logger logger = LoggerFactory.getLogger(UGCWikiWebAuthInterceptor.class);
    ...
    protected boolean handle(HttpServletRequest request, HttpServletResponse response, ResultWap result) {
        ...
        try {
            ...
            if (LoginUtil.logindScrKeyExists(request)) {
                response.sendRedirect("http://passport." + WebappConfig.get().getDomain() + "/ugcwiki/auth/loginpage?reurl=" + URLEncoder.encode(urlPathHelper.getRequestUri(request) + "?" + queryString, "UTF-8"));
            } else {
                response.sendRedirect("http://passport." + WebappConfig.get().getDomain() + "/ugcwiki/auth/loginpage?reurl=" + URLEncoder.encode(urlPathHelper.getRequestUri(request) + "?" + queryString, "UTF-8"));
            }
        } catch (IOException e) {
