define(function (require, exports, module) {
    ...
    $().ready(function () {
        if ($("div[id^=block_]").length > 0) {
            loadKey = true;
        }
        $("div[id^=block_]").each(function (i) {
            if (offSetIndex == 0) {
    ...
    (function () {
        ...
        var rightPosition = parseInt(($(document.body).width() - 933) / 2);
        if ($(document.body).width() > 933) {
            $("#box").css('right', rightPosition - 55 + 'px');
        } else {
            $("#box").css('right', 0 + 'px');
        }
        ...
        $("#downPage").live('click', function () {
            ...
            if (index + 1 == offSetTop.length) {
                if (loadKey) {
                    discovery.initImageDisplay(seeIndex, magazineCode);
                }
                var _this = $(this)
...
//            $("html,body").animate({scrollTop:0}, 450);
    ...
    var scrollOnWall = function () {
        ...
        if (isFoot && loadKey && $("div[id^=block_]").length <= 100) {
            discovery.initImageDisplay(seeIndex, magazineCode);
        }
    }
    ...
    var reScrollTopLeft = function () {
        if ($(window).width() > 933) {
            $("#box").css("right", ($(window).width() - 933) / 2 - 55 + 'px');
        } else {
            $("#box").css("right", 0);
        }
    }
