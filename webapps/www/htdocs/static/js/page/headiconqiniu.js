define(function (require, exports, module) {
    ...
    var headicon = {
        ...
        initHeadIcon: function (imgSrc) {
            ...
            $('.custom-img-advance').attr('display', 'block');
        },
        //图片上传成功后的设置
        ...
        resetUpLoad: function () {
            if ($('#.set_avatar_show ul li[name=delHeadicon]').size() < 8) {
                $("#upButton").attr("display", "block");
                $("#szlistBtn").show();
            } else {
                $("#upButton").attr("display", "none");
                $("#szlistBtn").hide();
    ...
    var thumbnailHeadIcon = function (loadingCallback, callback, completeCallback, errorCallback) {
        thumbnail($("#tumName").val(), $('#x1').val(), $('#y1').val(), $('#x2').val(), $('#y2').val(), $('#ow').val(), $('#oh').val(), loadingCallback, callback, completeCallback, errorCallback);
    };

    var thumbnail = function (img, x1, y1, x2, y2, w, h, loadingCallback, callback, completeCallback, errorCallback) {
        if (window.savethumbLock) {
            return false;
        }
        window.savethumbLock = true;
        var thumImg = img;
        if (img.indexOf('?imageView2') > 0) {
            thumImg += "|imageMogr2/crop/!" + x2 + "x" + y2 + "a" + x1 + "a" + y1;
        } else {
            thumImg += "?imageMogr2/crop/!" + x2 + "x" + y2 + "a" + x1 + "a" + y1;
        }
        $.ajax({
            url: joyconfig.urlUpload + "/json/upload/figureurl",
            type: 'post',
            data: {url: thumImg, token: joyconfig.token, appkey: 'default', uno: joyconfig.joyuserno},
            dataType: "jsonp",
            jsonpCallback: "figureurlcallback",
...
//                common.genImgDomain(jqObj.result[0].m, joyconfig.DOMAIN);
    ...
    var showHeadIconRegOk = function (jsonObj) {
        if (jsonObj.status_code == '-1') {
            ...
            jcrop_api.destroy();
            $('#headIconSetDiv').hide();
            $('#headIconDiv').attr("display", "block");
            //同步数据库
