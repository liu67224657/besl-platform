...
function commentAlert(obj) {
    if (window.uno == null || window.uid == null) {
        ...
        var msg = '请先<a href="javascript:loginDiv();">登录</a>后<br />再进行操作';
        tips(msg);
        return;
    }
...
function getCommentListCallBack(commentObj, unikey, domain, ishot) {
    ...
    var html = "<div id='" + hotid + "cont_cmt_list_" + reply.reply.rid + "' class='module-list' " +
        ...
        "' class='module-zan fr " + zanclass + "' onclick='dianzan(this)'>" + reply.reply.agree_sum + "</span></div>" +
        "</div></div>" +
        "<div class='module-txt'>" + reply.reply.body.text + "</div><div class='module-date'><span>" + reply.reply.post_date + "</span>" + removeHtml + "</div>" + (window.wikipage ? '' : reCommentListHtml) + "</div>";
    return html;
}

function postComment(unikey, domain, body, oid, pid) {
    if (window.uno == null || window.uid == null) {
        var msg = '请先<a href="' + window.login + '?channel=' + window.channel + '&reurl=' + encodeURIComponent(document.location.href) + '">登录</a>后<br />再进行操作';
        tips(msg);
        return;
    }
    $(".wp-bg").hide();
            ...
            if (resMsg.rs == '-1') {
                ...
                if (oid == 0) {// 主楼评论
                    ...
                    var commentHtml = "<div id='cont_cmt_list_" + result.reply.rid + "' class='module-list'><div class='module-infobox'>" +
                        ...
                        result.reply.agree_sum + "</span></div>" +
                        "</div></div>" +
                        "<div class='module-txt'>" + result.reply.body.text + "</div><div class='module-date'><span>" + result.reply.post_date + "</span>" + removeHtml + "</div><div class='module-double' id='children_reply_list_" +
...
function scroll_load() {
    ...
    $("#outer-div").scroll(function (ev) {
        ...
        if (sTop >= sNum && window.pnum <= window.ptotal) {
