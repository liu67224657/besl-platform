...
function lz_main(fid) {
    ...
    document.cookie = 'expires=' + new Date(Date.parse(new Date()) + 1000 * 60 * 60 * 24 * 30).toGMTString();
...
function lz_main(fid, stats) {
    ...
    if (stats) {
        var cgi = joyconfig.URL_WWW + "/pv";
        var dest = cgi + "?&ref=" + ref + "&domain=" + domain + "&app=" + app +
                "&browserVersion=" + browser_version + "&os=" + os + "&lid=" + locationId + "&screen=" + screen;
