...
(function() {
    ...
    if (!window.CKEDITOR)window.CKEDITOR = (function() {
        var a = {timestamp:'B49E5BQ',version:'3.6',revision:'6902',_:{},status:'unloaded',basePath:(function() {
    ...
    (function() {
        var l = e.extend({table:1,ul:1,ol:1,dl:1}, f.table, f.ul, f.ol, f.dl),m = {ol:1,ul:1},n = e.extend({}, {html:1}, f.html, f.body, f.head, {style:1,script:1});
        a.htmlParser.fragment.fromHtml = function(o, p, q) {
            var r = new a.htmlParser(),s = q || new a.htmlParser.fragment(),t = [],u = [],v = s,w = false;

            function x(A) {
                var B;
                if (t.length > 0)for (var C = 0; C < t.length; C++) {
                    var D = t[C],E = D.name,F = f[E],G = v.name && f[v.name];
            ...
            function z(A, B, C) {
                ...
                if (p && (!B.type || B.name == 'body')) {
                    var E,F;
                    if (A.attributes && (F = A.attributes['data-cke-real-element-type']))E = F; else E = A.name;
                ...
                if (A._.isBlockLike && A.name != 'pre') {
                    var G = A.children.length,H = A.children[G - 1],I;
                    if (H && H.type == 3)if (!(I = e.rtrim(H.value)))A.children.length = G - 1; else H.value = I;
                }
                B.add(A);
                if (A.returnPoint) {
                    v = A.returnPoint;
            ...
            r.onTagOpen = function(A, B, C, D) {
                ...
                if (f.$removeEmpty[A]) {
                    t.push(E);
                    return;
                } else if (A == 'pre')w = true; else if (A == 'br' && w) {
                ...
                while (1) {
                    var F = v.name,G = F ? f[F] || (v._.isBlockLike ? f.div : f.span) : n;
                    if (!E.isUnknown && !v.isUnknown && !G[A]) {
                        if (v.isOptionalClose)r.onTagClose(F); else if (A in m && F in m) {
                            var H = v.children,I = H[H.length - 1];
                            if (!(I && I.name == 'li'))z(I = new a.htmlParser.element('li'), v);
                            !E.returnPoint && (E.returnPoint = v);
                            v = I;
                        } else if (A in f.$listItem && F != A)r.onTagOpen(A == 'li' ? 'ul' : 'dl', {}, 0, 1); else if (F in l && F != A) {
                            !E.returnPoint && (E.returnPoint = v);
                            v = v.parent;
                        } else {
                            if (F in f.$inline)t.unshift(v);
                            if (v.parent)z(v, v.parent, 1); else {
                                E.isOrphan = 1;
                                break;
                            }
                        }
        ...
        a.htmlParser.fragment.prototype = {add:function(o) {
            var r = this;
            var p = r.children.length,q = p > 0 && r.children[p - 1] || null;
            if (q) {
    ...
    (function() {
        ...
        d.range.prototype = {clone:function() {
            ...
            if (!this.collapsed) {
                ...
                if (w && w.type == 3)if (!y)v.setStartBefore(w); else if (y >= w.getLength())v.setStartAfter(w);
                else {
                    v.setStartBefore(w);
                    B = 0;
                }
    ...
    (function() {
        k.button.prototype = {render:function(m, n) {
            ...
            n.push(' onkeydown="return CKEDITOR.tools.callFunction(', u, ', event);" onfocus="return CKEDITOR.tools.callFunction(', v, ', event);" onclick="CKEDITOR.tools.callFunction(', s, ', this); return false;"><span class="cke_icon"');
            if (this.icon) {
    ...
    (function() {
        j.add('enterkey', {requires:['keystrokes','indent'],init:function(t) {
                    ...
                    t.addCommand('ctrlEnter', {modes:{wysiwyg:1},editorFocus:false,exec:function(v) {
                                if (document.getElementById("post_text_submit")!=null) {
                                    if(window.posttextlock===false){
                                        document.getElementById("post_text_submit").click();
                                    }

                                    }
                                if(document.getElementById("edit_text_submit")!=null){
                                    document.getElementById("edit_text_submit").click();
                                }
                            }});
    ...
    (function() {
        ...
        var B = /<(a|area|img|input)\b([^>]*)>/gi,C = /\b(href|src|name|joymet|joymeu|joymed|joymef)\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|(?:[^ "'>]+))/gi,D = /(?:<style(?=[ >])[^>]*>[\s\S]*<\/style>)|(?:<(:?link|meta|base)[^>]*>)/gi,E = /<cke:encoded>([^<]*)<\/cke:encoded>/gi,F = /(<\/?)((?:object|embed|param|html|body|head|title)[^>]*>)/gi,G = /(<\/?)cke:((?:html|body|head|title)[^>]*>)/gi,H = /<cke:(param|embed)([^>]*?)\/?>(?!\s*<\/cke:\1)/gi;

        function I(T) {
            return T.replace(B, function(U, V, W) {
                return '<' + V + W.replace(C, function(X, Y) {
                    if (W.indexOf('data-cke-saved-' + Y) == -1)return ' data-cke-saved-' + X + ' ' + X;
                    return X;
                }) + '>';
            });
        }
...
//                });
    ...
    i.keystrokes = [
        ...
        [1000 + 73,'italic'],
        [1000 + 85,'underline'],
        [4000 + (c || b.webkit ? 189 : 109),'toolbarCollapse'],
        [4000 + 48,'a11yHelp']
    ];
    j.add('link', {init:function(m) {
                ...
                if (m.contextMenu)m.contextMenu.addListener(function(p, q) {
                    ...
                    if (r && r.hasAttribute('name'))s.anchor = 2;
                    return s;
                }
                );
            },afterInit:function(m) {
    ...
    (function() {
        ...
        j.list = {listToArray:function(E, F, G, H, I) {
            ...
            for (var J = 0,K = E.getChildCount(); J < K; J++) {
                var L = E.getChild(J);
                if (L.type == 1 && L.getName() in f.$list)j.list.listToArray(L, F, G, H + 1);
            ...
            var J = E[G].parent.getDocument(),K = new d.documentFragment(J),L = null,M = G,N = Math.max(E[G].indent, 0),O = null,P,Q = H == 1 ? 'p' : 'div';
            while (1) {
                var R = E[M];
                if (R.indent == N) {
                    ...
                    O = L.append(R.element.clone(0, 1));
                    for (var S = 0; S < R.contents.length; S++)O.append(R.contents[S].clone(1, 1));
                    M++;
                } else if (R.indent == Math.max(N, 0) + 1) {
                    var T = R.element.getDirection(1),U = E[M - 1].element.getDirection(1),V = j.list.arrayToList(E, null, M, H, U != T ? T : null);
                    if (!O.getChildCount() && c && !(J.$.documentMode > 7))O.append(J.createText('\xa0'));
                    O.append(V.listNode);
                    M = V.nextIndex;
                } else if (R.indent == -1 && !G && R.grandparent) {
                    if (m[R.grandparent.getName()]) {
                        O = R.element.clone(false, true);
                        P = R.element.getDirection(1);
                        R.grandparent.getDirection(1) != P && O.setAttribute('dir', P);
                    } else if (I || R.element.hasAttributes() || H != 2) {
                        O = J.createElement(Q);
                        R.element.copyAttributes(O, {type:1,value:1});
                        P = R.element.getDirection() || I;
                        P && O.setAttribute('dir', P);
                        if (!I && H == 2 && !O.hasAttributes())O = new d.documentFragment(J);
                    } else O = new d.documentFragment(J);
                    for (S = 0; S < R.contents.length; S++)O.append(R.contents[S].clone(1, 1));
                    if (O.type == 11 && M != E.length - 1) {
                        var W = O.getLast();
                        if (W && W.type == 1 && W.getAttribute('type') == '_moz')W.remove();
                        if (!(W = O.getLast(q) && W.type == 1 && W.getName() in f.$block))O.append(J.createElement('br'));
                    }
                    if (O.type == 1 && O.getName() == Q && O.$.firstChild) {
                        O.trim();
                        var X = O.getFirst();
                        if (X.type == 1 && X.isBlockBoundary()) {
                            var Y = new d.documentFragment(J);
                            O.moveChildren(Y);
                            O = Y;
                        }
                    }
                    var Z = O.$.nodeName.toLowerCase();
                    if (!c && (Z == 'div' || Z == 'p'))O.appendBogus();
                    K.append(O);
                    L = null;
                    M++;
                } else return null;
            ...
            if (F) {
                var aa = K.getFirst();
                while (aa) {
                    if (aa.type == 1)h.clearMarkers(F, aa);
        ...
        w.prototype = {exec:function(E) {
            var F = E.document,G = E.config,H = E.getSelection(),I = H && H.getRanges(true);
            if (!I || I.length < 1)return;
            if (this.state == 2) {
                var J = F.getBody();
                if (!J.getFirst(q)) {
                    G.enterMode == 2 ? J.appendBogus() : I[0].fixBlock(1, G.enterMode == 1 ? 'p' : 'div');
                    H.selectRanges(I);
                } else {
                    var K = I.length == 1 && I[0],L = K && K.getEnclosedNode();
                    if (L && L.is && this.type == L.getName())this.setState(1);
                }
            }
            var M = H.createBookmarks(true),N = [],O = {},P = I.createIterator(),Q = 0;
    ...
    (function() {
        ...
        j.add('maximize', {init:function(s) {
                    ...
                    ;
                    var B = 2;
                    s.addCommand('maximize', {modes:{wysiwyg:1,source:1},readOnly:1,editorFocus:false,exec:function() {
    ...
    j.pagebreakCmd = {exec:function(m) {
        var n = m.lang.pagebreakAlt,o = h.createFromHtml('<div style="page-break-after: always;"contenteditable="false" title="' + n + '" ' + 'aria-label="' + n + '" ' + 'data-cke-display-name="pagebreak" ' + 'class="cke_pagebreak">' + '</div>'),p = m.getSelection().getRanges(true);
        m.fire('saveSnapshot');
    ...
    (function() {
        ...
        function B(G, H, I) {
            ...
            for (var K = 0; K < G.length; K++) {
                ...
                else if (I.is && L[H] == I.$)return K; else if (K == I)return new h(L[H]);
            }
    ...
    (function() {
        ...
        function q(C) {
            var I = this;
            if (I.dataProcessor)C = I.dataProcessor.toHtml(C);
            var D = I.getSelection(),E = D.getRanges()[0];
            if (E.checkReadOnly())return;
            if (c) {
                var F = D.isLocked;
                if (F)D.unlock();
                var G = D.getNative();
                if (G.type == 'Control')G.clear(); else if (D.getType() == 2) {
                    E = D.getRanges()[0];
                    var H = E && E.endContainer;
                    if (H && H.type == 1 && H.getAttribute('contenteditable') == 'false' && E.checkBoundaryOfElement(H, 2)) {
                        E.setEndAfter(E.endContainer);
                        E.deleteContents();
                    }
                }
                try {
                    G.createRange().pasteHTML(C);
                } catch(J) {
                }
                if (F)I.getSelection().lock();
            } else I.document.$.execCommand('inserthtml', false, C);
            if (b.webkit) {
                D = I.getSelection();
                D.scrollIntoView();
            }
        ...
        j.add('wysiwygarea', {requires:['editingblock'],init:function(C) {
                    ...
                    C.on('editingBlockReady', function() {
                        var L,M,N,O,P,Q,R = b.isCustomDomain(),S = function(V) {
                            ...
                            M.on('load', function(aa) {
                                P = 1;
                                aa.removeListener();
                                var ab = M.getFrameDocument();
                                ab.write(V);
                                b.air && U(ab.getWindow().$);
                            });
                            if (document.location.protocol == 'chrome:')a.event.useCapture = false;
                            var X = C.element,Y = b.gecko && !X.isVisible(),Z = {};
                            if (Y) {
                                X.show();
                                Z = {position:X.getStyle('position'),top:X.getStyle('top')};
                                X.setStyles({position:'absolute',top:'-3000px'});
                            }
                            L.append(M);
                        ...
                        function U(V) {
                            ...
                            if (c) {
                                ...
                                Z && W.on('keydown', function(af) {
                                    ...
                                    if (ag in {8:1,46:1}) {
                                        var ah = C.getSelection(),ai = ah.getSelectedElement();
                                        if (ai) {
                                            C.fire('saveSnapshot');
                                ...
                                C.config.enterMode != 1 && W.on('selectionchange', function() {
                        ...
                        C.addMode('wysiwyg', {load:function(V, W, X) {
                                    ...
                                    var V = C.window;
                                    if (N)O = true; else if (b.opera && C.document) {
    ...
    (function() {
        ...
        a.dialog = function(Q, R) {
            var S = a.dialog._.dialogDefinitions[R],T = e.clone(s),U = Q.config.dialog_buttonsOrder || 'OS',V = Q.lang.dir;
            if (U == 'OS' && b.mac || U == 'rtl' && V == 'ltr' || U == 'ltr' && V == 'rtl')T.buttons.reverse();
            S = e.extend(S(Q), T);
            S = e.clone(S);
            S = new w(this, S);
            var W = a.document,X = Q.theme.buildDialog(Q);
            this._ = {editor:Q,element:X.element,name:R,contentSize:{width:0,height:0},size:{width:0,height:0},contents:{},buttons:{},accessKeyMap:{},tabs:{},tabIdList:[],currentTabId:null,currentTabIndex:null,pageCount:0,lastTab:null,tabBarMode:false,focusList:[],currentFocusIndex:0,hasFocus:false};
            this.parts = X.parts;
            e.setTimeout(function() {
                Q.fire('ariaWidget', this.parts.contents);
            }, 0, this);
            var Y = {position:b.ie6Compat ? 'absolute' : 'fixed',top:0,visibility:'hidden'};
            ...
            if (Q._.removeDialogTabs && (Z = Q._.removeDialogTabs[R]))for (i = 0; i < Z.length; i++)S.removeContents(Z[i]);
            ...
            var ad = this,ae = function(ap) {
                var aq = ad._.contents,ar = false;
                for (var as in aq)for (var at in aq[as]) {
            ...
            this.on('ok', function(ap) {
                ae(function(aq) {
                    if (aq.validate) {
                        ...
                        if (ar === false) {
                            if (aq.select)aq.select(); else aq.focus();
                            ap.data.hide = false;
                            ap.stop();
                            return true;
                        }
            ...
            function af() {
                ...
                ap.sort(function(as, at) {
                    if (as.tabIndex != at.tabIndex)return at.tabIndex - as.tabIndex; else return as.focusIndex - at.focusIndex;
                });
                var aq = ap.length;
            ...
            ;
            function ag(ap) {
                var aq = ad._.focusList,ar = ap ? 1 : -1;
                ...
                var at = (as + ar + aq.length) % aq.length,au = at;
            ...
            function ai(ap) {
                ...
                var aq = ap.data.getKeystroke(),ar = Q.lang.dir == 'rtl';
                ah = 0;
                if (aq == 9 || aq == 2000 + 9) {
                    var as = aq == 2000 + 9;
                    if (ad._.tabBarMode) {
            ...
            this.on('show', function() {
                ...
                if (Q.config.dialog_startupFocusTab && ad._.pageCount > 1) {
                    ...
                    if (at._.editor.mode == 'wysiwyg' && c) {
                        var aq = Q.document.$.selection,ar = aq.createRange();
                        if (ar)if (ar.parentElement && ar.parentElement().ownerDocument == Q.document.$ || ar.item && ar.item(0).ownerDocument == Q.document.$) {
            ...
            if (b.ie6Compat)this.on('load', function(ap) {
                var aq = this.getElement(),ar = aq.getFirst();
                ar.remove();
                ar.appendTo(aq);
            }, this);
            y(this);
            ...
            var an = [],ao = a.dialog._.uiElementBuilders.hbox.build(this, {type:'hbox',className:'cke_dialog_footer_buttons',widths:[],children:S.buttons}, an).getChild();
        ...
        function r(Q, R, S) {
            ...
            R.on('keydown', function(T) {
                if (T.data.getKeystroke() in {32:1,13:1})this.fire('click');
            });
            R.on('focus', function() {
                this.fire('mouseover');
            });
        ...
        a.dialog.prototype = {destroy:function() {
            ...
            if (a.dialog._.currentTop === null) {
                ...
                for (var T in {keyup:1,keydown:1,keypress:1})Q.on(T, P);
            } else {
            ...
            var T = this;
            for (var R in T._.contents)for (var S in T._.contents[R])Q(T._.contents[R][S]);
            return T;
        },reset:(function() {
            var Q = function(R) {
            ...
            if (!this._.parentDialog) {
                ...
                for (var S in {keyup:1,keydown:1,keypress:1})Q.removeListener(S, P);
                var T = this._.editor;
                T.focus();
                if (T.mode == 'wysiwyg' && c) {
            ...
            if (this.fire('selectPage', {page:Q,currentPage:this._.currentTabId}) === true)return;
            ...
            if (!R || S._.pageCount == 1 || !R.isVisible())return; else if (Q == S._.currentTabId)S.selectPage(o.call(S));
            R.hide();
            S._.pageCount--;
            S.updateStyle();
        },showPage:function(Q) {
        ...
        e.extend(a.dialog, {add:function(Q, R) {
                    ...
                    var Q = function(R, S) {
                        ...
                        return e.extend({id:'ok',type:'button',label:R.lang.common.ok,'class':'cke_dialog_ui_button_ok',onClick:function(T) {
                                    var U = T.data.dialog;
                                    if (U.fire('ok', {hide:true}).hide !== false)U.hide();
                                }}, S, true);
                    };
                    Q.type = 'button';
                    ...
                    var Q = function(R, S) {
                        ...
                        return e.extend({id:'cancel',type:'button',label:R.lang.common.cancel,'class':'cke_dialog_ui_button_cancel',onClick:function(T) {
                                    var U = T.data.dialog;
                                    if (U.fire('cancel', {hide:true}).hide !== false)U.hide();
                                }}, S, true);
                    };
                    Q.type = 'button';
                    Q.override = function(R) {
        ...
        var s = {resizable:3,minWidth:600,minHeight:400,buttons:[a.dialog.okButton,a.dialog.cancelButton]},t = function(Q, R, S) {
        ...
        w.prototype = {getContents:function(Q) {
        ...
        function y(Q) {
            ...
            function X(Z) {
                ...
                S.x += ae;
                S.y += af;
                if (S.x + W[3] < V)ag = -W[3]; else if (S.x - W[1] > ab.width - aa.width - V)ag = ab.width - aa.width + (U.lang.dir == 'rtl' ? 0 : W[1]); else ag = S.x;
                if (S.y + W[0] < V)ah = -W[0]; else if (S.y - W[2] > ab.height - aa.height - V)ah = ab.height - aa.height + W[2]; else ah = S.y;
                Q.move(ag, ah, 1);
        ...
        function z(Q) {
            ...
            Q.on('load', function() {
                var ad = '';
                if (S == 1)ad = ' cke_resizer_horizontal'; else if (S == 2)ad = ' cke_resizer_vertical';
        ...
        ;
        function E(Q) {
            var R = a.document.getWindow(),S = Q.config,T = S.dialog_backgroundCoverColor || 'white',U = S.dialog_backgroundCoverOpacity,V = S.baseFloatZIndex,W = e.genKey(T, U, V),X = B[W];
            if (!X) {
        ...
        ;
        var H = {},I = function(Q) {
            var R = Q.data.$.ctrlKey || Q.data.$.metaKey,S = Q.data.$.altKey,T = Q.data.$.shiftKey,U = String.fromCharCode(Q.data.$.keyCode),V = H[(R ? 'CTRL+' : '') + (S ? 'ALT+' : '') + (T ? 'SHIFT+' : '') + U];
            ...
            k.dialog = {uiElement:function(Q, R, S, T, U, V, W) {
                ...
                var V = this._.children = R,W = U && U.width || null,X = U && U.heights || null,Y = function() {
                    ...
                    for (var aa = 0; aa < S.length; aa++) {
                        ...
                        if (X)ab.push('height:' + m(X[aa])); else if (U && U.expand)ab.push('height:' + Math.floor(100 / S.length) + '%');
        ...
        k.dialog.uiElement.prototype = {getElement:function() {
            ...
            for (var U in Q) {
                ...
                if (this.eventProcessors[U])this.eventProcessors[U].call(this, this._.dialog, Q[U]); else T(this, this._.dialog, S[1].toLowerCase(), Q[U]);
            }
            return this;
        ...
        (function() {
            ...
            a.dialog.validate = {functions:function() {
                return function() {
                    var Z = this;
                    var T = Z && Z.getValue ? Z.getValue() : arguments[0],U = undefined,V = 2,W = [],X;
                    for (X = 0; X < arguments.length; X++) {
                        if (typeof arguments[X] == 'function')W.push(arguments[X]); else break;
                    }
                    ...
                    for (X = 0; X < W.length; X++) {
                        if (V == 2)Y = Y && W[X](T); else Y = Y || W[X](T);
                    }
                    if (!Y) {
                ...
                return function() {
                    ...
                    if (!T.test(V)) {
                        if (U !== undefined)alert(U);
                        if (W && (W.select || W.focus))if (W.select)W.select(); else W.focus();
                        return false;
                    }
                    return true;
                };
    ...
    a.STYLE_OBJECT = 3;
    (function() {
        var m = {address:1,div:1,h1:1,h2:1,h3:1,h4:1,h5:1,h6:1,p:1,pre:1},n = {a:1,embed:1,hr:1,img:1,li:1,object:1,ol:1,table:1,td:1,tr:1,th:1,ul:1,dl:1,dt:1,dd:1,form:1},o = /\s*(?:;\s*|$)/,p = /#\((.+?)\)/g,q = d.walker.bookmark(0, 1),r = d.walker.whitespaces(1);
        ...
        function t(T) {
            ...
            T.enlarge(1, 1);
            T.trim();
            var ab = T.createBookmark(),ac = ab.startNode,ad = ab.endNode,ae = ac,af,ag = s(ac),ah = s(ad);
            ...
            while (ae) {
                ...
                if (ai && af && !af.collapsed) {
                    var aq = J(ax, U),ar = aq.hasAttributes(),as = af.getCommonAncestor(),at = {styles:{},attrs:{},blockedStyles:{},blockedAttrs:{}},au,av,aw;
                    while (aq && as) {
                        if (as.getName() == W) {
    ...
    (function() {
        ...
        function r(y) {
            ...
            ;
            var A = y.startContainer,B = y.startOffset;
            ...
            return function(z) {
                ...
                if (z) {
                    ...
                    for (var C = 0; C < B.length; C++) {
                        ...
                        var F = D.startContainer,G = D.endContainer,H = D.startOffset,I = D.endOffset,J = D.clone(),K;
                        if (K = F.isReadOnly())D.setStartAfter(K);
                        if (F && F.type == 3)if (H >= F.getLength())J.setStartAfter(F); else J.setStartBefore(F);
                        if (G && G.type == 3)if (!I)J.setEndBefore(G); else J.setEndAfter(G);
    ...
    k.richCombo = e.createClass({$:function(m) {
                ...
                var o = b,p = 'cke_' + this.id,q = e.addFunction(function(u) {
                    var x = this;
                    var v = x._;
                    if (v.state == 0)return;
                    x.createPanel(m);
                    ...
                    x.commit();
                    var w = x.getValue();
                    if (w)v.list.mark(w); else v.list.unmarkAll();
                ...
                n.push(' onkeydown="CKEDITOR.tools.callFunction( ', t, ', event, this );" onclick="CKEDITOR.tools.callFunction(', q, ', this); return false;"><span><span id="' + p + '_text" class="cke_text cke_inline_label">' + this.label + '</span>' + '</span>' + '<span class=cke_openbutton><span class=cke_icon>' + (b.hc ? '&#9660;' : b.air ? '&nbsp;' : '') + '</span></span>' + '</a>' + '</span>' + '</span>');
                if (this.onRender)this.onRender();
                return r;
