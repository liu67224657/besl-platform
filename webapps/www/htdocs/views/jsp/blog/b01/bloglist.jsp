...
<head>
    ...
    <title>${profile.blog.screenName}_${jmh_title}</title>
...
<div class="con_tab clearfix">
    <ul>
        <li>
            <a class="on" href="${URL_WWW}/people/${profile.blog.domain}">
                <span>全部动态</span>
            </a>
        </li>
        <li>
            <a href="${URL_WWW}/profile/favorite/${profile.blog.uno}">
                <span>
                    <c:choose><c:when test="${userSession!=null && userSession.blogwebsite.uno==profile.blog.uno}">我</c:when><c:when test="${profile.detail.sex!=null && profile.detail.sex eq '0'}">她</c:when><c:otherwise>他</c:otherwise></c:choose>的喜欢
                </span>
            </a>
        </li>
        <li>
            <a href="${URL_WWW}/profile/experience/${profile.blog.uno}">
                <span>个人资料</span>
            </a>
        </li>
    </ul>
...
<div class="con_wrap">
    ...
    <div class="con_area con_l clearfix">
        <div class="boke_bd">
            <a href="${URL_WWW}/people/<c:out value='${profile.blog.domain}'/>"
               <c:if test="${subtype==null}">class="on"</c:if>>全部
            </a>
            <a href="${URL_WWW}/people/<c:out value='${profile.blog.domain}'/>?subtype=org"
               <c:if test="${subtype=='org'}">class="on"</c:if>> 主帖
            </a>
            <a href="${URL_WWW}/people/<c:out value='${profile.blog.domain}'/>?subtype=p"
               <c:if test="${subtype=='p'}">class="on"</c:if>> 图片
            </a>
            <a href="${URL_WWW}/people/<c:out value='${profile.blog.domain}'/>?subtype=v"
               <c:if test="${subtype=='v'}">class="on"</c:if>>视频
            </a>
            <a href="${URL_WWW}/people/<c:out value='${profile.blog.domain}'/>?subtype=m"
               <c:if test="${subtype=='m'}">class="on"</c:if>> 音乐
            </a>
        </div>
        <c:if test="${fn:length(blogList)<1}">
            <div class="addgameshowlist companny_mg clearfix">
                ...
                <c:choose>
                    <c:when test="${subtype==null}">
                        <c:choose>
                            <c:when test="${userSession!=null && userSession.blogwebsite.uno==profile.blog.uno}">
                                <fmt:message key="blog.my.empty" bundle="${userProps}">
                                    <fmt:param value="${URL_WWW}/home"/>
                                </fmt:message>
                            </c:when>
                            <c:otherwise>
                                <fmt:message key="blog.other.empty" bundle="${userProps}">
                                    <fmt:param value="${who}"/>
                                </fmt:message>
                            </c:otherwise>
                        </c:choose>
                    </c:when>
                    <c:when test="${subtype eq 'org'}">
                        <c:choose>
                            <c:when test="${userSession!=null && userSession.blogwebsite.uno==profile.blog.uno}">
                                <fmt:message key="blog.my.org.empty" bundle="${userProps}">
                                    <fmt:param value="${URL_WWW}/home"/>
                                </fmt:message>
                            </c:when>
                            <c:otherwise>
                                <fmt:message key="blog.other.org.empty" bundle="${userProps}">
                                    <fmt:param value="${who}"/>
                                </fmt:message>
                            </c:otherwise>
                        </c:choose>
                    ...
                    <c:otherwise>
                        <fmt:message key="blog.${subtype}.empty" bundle="${userProps}">
                            <fmt:param value="${who}"/>
                        </fmt:message>
                    </c:otherwise>
                </c:choose>
                    ...
                    <dl>
                        <dd class="textcon">
                            ...
                            <c:if test="${blogContent.profile.detail.verifyType !=null && blogContent.profile.detail.verifyType.code != 'n'}">
                                <a href="${URL_WWW}/people/${blogContent.profile.blog.domain}" class="${blogContent.profile.detail.verifyType.code}vip" title="<fmt:message key="verify.profile.${blogContent.profile.detail.verifyType.code}" bundle="${userProps}"/>"></a>
                            </c:if>
                                    ...
                                    <c:if test="${fn:length(blogContent.content.images.images)>0}">
                                        <c:choose>
                                            <c:when test="${fn:length(blogContent.content.images.images)==1}">
                                                <div class="single_pic" id="img_${blogContent.content.contentId}" data-cid="${blogContent.content.contentId}" data-cuno="${blogContent.content.uno}">
                                        ...
                                        <div class="discuss_area clearfix">
                                            ...
                                            <c:choose>
                                                <c:when test="${blogContent.rootContent!=null && !(blogContent.rootContent.removeStatus.code eq 'y')}">
                                                    ...
                                                    <div class="area_ft">
                                                        <span class="time">
                                                            ${dateutil:parseDate(blogContent.rootContent.publishDate)}
                                                        </span>
                                                        <span class="from">
                                                           <c:choose>
                                                            ...
                                                            <ul>
                                                                ...
                                                                <li>
                                                                    <a href="${URL_WWW}/note/${blogContent.rootContent.contentId}"
                                                                       class="share" title="评论">评论<c:if test="${blogContent.rootContent.replyTimes>0}">(<span name="ia_header_num_reply_${blogContent.rootContent.contentId}">${blogContent.rootContent.replyTimes}</span>)</c:if></a></li>
                                                                <li>
                                ...
                                <div class="operate">
                                    ...
                                    <c:if test="${userSession!=null && blogContent.content.uno eq userSession.blogwebsite.uno}">
                                        ...
                                        <c:choose>
                                            ...
                                            <c:otherwise>
                                                <li><a href="${ctx}/content/edit/${blogContent.content.uno}/${blogContent.content.contentId}"
                                                   class="listedit" title="编辑">编辑</a><em>|</em></li>
                                            </c:otherwise>
                                        </c:choose>
                                        </c:if>
                                        ...
                                        <li><a href="javascript:void(0);" name="replyLink" class="share" title="评论" data-itype="reply"
                                               ...
                                               data-cuno="${blogContent.content.uno}">评论<c:if test="${blogContent.content.replyTimes>0}">(<span name="ia_header_num_reply_${blogContent.content.contentId}">${blogContent.content.replyTimes}</span>)</c:if></a></li>
                                        <li>
        ...
        <c:set var="pageurl" value="${URL_WWW}/people/${profile.blog.domain}"/>
        <c:if test="${fn:length(subtype)>0}">
        <c:set var="pageparam" value="subtype=${subtype}"/>
        </c:if>
...
<script type="text/javascript">
    ...
    var desc = '${fn:replace(profile.blog.description,vEnter,"")}';
