    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('#page_but').click(function(e){
                ...
                pageLock=true;
                pagenum=pagenum+1;
                var data= {'aid': aid, 'pagenum': pagenum};
                if(uno!=null){
                    data.uno=uno;
                }
                $.ajax({
                    url: '/json/kada/activity/contentpage',
                    type: 'post',
                    data: data,
                    dataType: "json",
                    success: function (data) {
                        if (data.rs == "1" && data.list!=null && data.list.length>0) {
                            $.each(data.list,function(i,val){
                                var article=val;

                                var html='<div><a href="joymekada://joymeapp?msgtype=4&info='+article.content.cid+'">'+
                                        '<div class="box">'+
                                        '<div><img src="'+article.profile.icon+'">'+article.profile.username+'</div>'+
                                        '<img class="t" src="'+article.content.pic.pic_s+'">'+
                                        '</div>'+
                                        '</a>'+
                                        '<a href="#" name="agree" data-cid="'+article.content.cid+'">'+
                                        '<div class="z-btn '+(article.content.isagree?'vv':'')+'" '+
                                        'data-cid="'+article.content.cid+'">'+article.content.agreenum+'</div></div>';

                                $('#article_list').append(html);
                            });

                            if(data.page!=null){
                                if(data.page.maxPage<=data.page.curPage){
                                    $('#page_but').parent().remove();
                                }
                            }
                        }
                    },
...
<article class="item2">
    <h2>活动照片</h2>
    <article class="piclist3" id="article_list">
...
<c:if test="${page.maxPage>1}">
    <div class="getmore2">
        <a href="javascript:;"  id="page_but">加载更多</a>
    </div>
</c:if>
