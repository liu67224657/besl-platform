...
document.write('<span><a href="${URL_WWW}/games" id="link_game_menu" <c:if
        test="${hdflag=='game'}">class="on"</c:if> >游戏 • WIKI</a></span>');
<%--<c:if test="${userSession!=null}">--%>
    <%--document.write('<span><a href="${URL_WWW}/home" <c:if--%>
        <%--test="${hdflag=='home'}">class="on"</c:if> >个人中心</a></span> ');--%>
<%--</c:if>--%>
document.write('<span><a href="${URL_WWW}/giftmarket" <c:if
        test="${hdflag=='giftmarket'}">class="on"</c:if> >礼包中心</a></span>');
document.write('<span><a href="http://html.joyme.com/mobile/gameguides.html">着迷APP</a></span>');
document.write('</div>');
...
<c:choose>
    <c:when test="${userSession!=null}">
        ...
        document.write('<img src="${uf:parseFacesInclude(userSession.icon,userSession.sex,"m" , true,0,1)[0]}"/>');
        document.write('</a>');
