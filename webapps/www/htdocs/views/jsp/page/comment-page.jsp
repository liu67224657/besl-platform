...
<c:if test="${page.maxPage>1}">
    <div class="page" style="${pagestyle}">
        <c:if test="${page.curPage>1}">
            <a href="${pageurl}pnum=${page.curPage-1}${anchorPoint}" class="prepage"><span>上一页</span></a>
        </c:if>
        <c:choose>
                ...
                <c:choose>
                        ...
                        <c:forEach var="curPage" begin="1" end="${page.maxPage}">
                            <c:choose>
                                <c:when test="${page.curPage!=curPage}">
                                    <a href="${pageurl}pnum=${curPage}${anchorPoint}"><span>${curPage}</span></a>
                                </c:when>
                                <c:otherwise>
                                    <b>${curPage}</b>
                                </c:otherwise>
                    ...
                    <c:otherwise>
                        ...
                        <c:if test="${page.curPage>=3}">
                            <c:if test="${page.curPage>3}">
                                <a href="${pageurl}pnum=${page.curPage+1}${anchorPoint}"><span>${page.curPage+1}</span></a>
                            </c:if>
                            <c:if test="${page.maxPage>=page.curPage+2}${anchorPoint}">
                                <a href="${pageurl}pnum=${page.curPage+2}${anchorPoint}"><span>${page.curPage+2}</span></a>
                            </c:if>
                        </c:if>
                        ...
                        <c:if test="${page.curPage<3||(page.curPage>=3 && page.maxPage>page.curPage+3)}">
                            <a href="${pageurl}pnum=${page.maxPage}${anchorPoint}"><span>${page.maxPage}</span></a>
                        </c:if>
                    </c:otherwise>
                </c:choose>
            </c:when>
            <c:when test="${page.curPage>4}">
                <a href="${pageurl}pnum=1${anchorPoint}"><span>1</span></a>
                <em>...</em>
                <c:choose>
        ...
        <c:choose>
            <c:when test="${page.curPage<page.maxPage}">
                <a href="${pageurl}pnum=${page.curPage+1}${anchorPoint}" class="nextpage"><span>下一页</span></a>
            </c:when>
