...
<div id="joyme-head-2013">
    <div class="head-2013-box clearfix">
        ...
        <div class="head-2013-left">
            <span><a href="${URL_WWW}"
                     <c:if test="${hdflag=='index'}">class="on"</c:if> >首页</a></span>
            <span><a href="http://wiki.joyme.com/"
                     <c:if test="${hdflag=='wiki'}">class="on"</c:if> >WIKI</a></span>
            <span><a href="${URL_WWW}/games"
                     <c:if test="${hdflag=='game'}">class="on"</c:if> >专区</a></span>
            <span><a href="${URL_WWW}/gift"
                     <c:if test="${hdflag=='giftmarket'}">class="on"</c:if> >礼包中心</a></span>
            <span><a href="http://html.joyme.com/mobile/gameguides.html">着迷APP</a></span>
                    ...
                    <div class="unlogin">
                        ...
                        <a href="http://passport.${DOMAIN}/auth/thirdapi/sinaweibo/bind<c:if test="${fn:indexOf(redr,'index')<0}">?reurl=${redr}</c:if>"
                           class="weibo" title="新浪微博登录" rel="nofollow"></a>
                            <%--<a href="http://passport.${DOMAIN}/auth/thirdapi/renren/bind<c:if test="${fn:indexOf(redr,'index')<0}">?rurl=${redr}</c:if>"--%>
                            <%--class="renren" title="人人登录" rel="nofollow"></a>--%>
