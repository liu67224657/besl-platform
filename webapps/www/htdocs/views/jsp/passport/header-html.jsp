...
<c:choose>
    <c:when test="${userSession!=null}">
        ...
        document.write('<img src="${icon:parseIcon(userSession.icon, userSession.sex, "")}"/>');
        document.write('</a>');
    ...
    <c:otherwise>
        ...
        document.write('<a href="http://passport.${DOMAIN}/auth/thirdapi/qq/bind?reurl='+escape(window.location.href)+'" class="qq" title="qq号码登录"></a>');
        document.write('<a href="http://passport.${DOMAIN}/auth/thirdapi/sinaweibo/bind?reurl='+escape(window.location.href)+'" class="weibo" title="新浪微博登录"></a>');
        <%--document.write('<a href="http://passport.${DOMAIN}/auth/thirdapi/renren/bind?reurl='+escape(window.location.href)+'" class="renren" title="人人登录"></a>');--%>
        document.write('<a href="http://passport.${DOMAIN}/auth/registerpage?reurl='+escape(window.location.href)+'">注册</a>');
        document.write('<a href="http://passport.${DOMAIN}/auth/loginpage?reurl='+escape(window.location.href)+'">登录</a>');
        document.write('</div>');
    </c:otherwise>
</c:choose>
document.write('</div>');
...
<c:if test="${userSession!=null}">
    ...
    document.write('<ul>');
    <%--document.write('<li class="first"><a href="${URL_WWW}/people/${userSession.domain}">我的博客</a></li>');--%>
    document.write('<li class="first"><a href="${URL_WWW}/profile/customize">设置</a></li>');
    document.write('<li>');
