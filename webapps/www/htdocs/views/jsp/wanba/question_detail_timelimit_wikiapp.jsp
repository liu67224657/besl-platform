...
<div style="padding-top:0px;" class="container <c:if test='${(timeout&&question.accepaid eq 0)|| (pid eq question.askprofileid&& question.accepaid eq 0&&question.reactivated eq 0)}'>suction-top</c:if>">
    <!-- 注释：无倒计时时去掉‘suction-top’类名，反之添加 -->
    <!-- 顶部悬浮 开始 -->
    <!-- 倒计时 -->
    <c:if test="${timeout&&question.accepaid eq 0}">
        <div class=" count-down suspend-ele" style="display: none;">
    ...
    <c:if test="${!timeout&&pid eq question.askprofileid && question.accepaid eq 0 &&question.reactivated eq 0}">
        <div class="restart suspend-ele" style="display:none;">
    ...
    <div id="wrapper">
        <div class="que-main border-bot">
            ...
            <c:if test="${timeout&&question.accepaid eq 0}">
                <div class="invite-icon" style="display: none;">
        ...
        <c:if test="${not empty gameAnimeTagMap[question.tagid]}">
            <div class="wiki-ans" style="display:none;">
                            ...
                            <c:if test="${not empty  answer.richbody}">
                                <c:choose>
                                    <c:when test="${answerpage.totalRows>1}">
                                        <p class="limit-line">${answer.richbody}</p></c:when>
                                    <c:otherwise>
                                        ${answer.richbody}
                                    </c:otherwise>
                                </c:choose>
                        ...
                        <div class="join-ans-item border-bot">
                            <div class="join-ans-cont">
                                ...
                                <c:if test="${not empty  answer.richbody}">
                                    <c:choose>
                                        <c:when test="${answerpage.totalRows>1}">
                                                <p class="limit-line">${answer.richbody}</p>
                                        </c:when>
                                        <c:otherwise>
                                            ${answer.richbody}
                                        </c:otherwise>
