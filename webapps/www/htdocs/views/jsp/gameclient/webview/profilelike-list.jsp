...
<head>
    ...
    <title><c:choose><c:when
            test="${fn:length(desuid) <= 0}">我喜欢的人${likesum}</c:when><c:otherwise>TA喜欢的人${likesum}</c:otherwise></c:choose>
    </title>
...
<div id="wrapper" style="overflow: scroll">
    <c:choose>
        <c:when test="${priflelist == null || priflelist.size() <= 0}">
            <!--TA喜欢的人 无数据-->
            <div class="like_none">
                <p><c:choose><c:when test="${fn:length(desuid) <= 0}">你还没有喜欢的人，去迷友中看看谁是你的菜。</c:when><c:otherwise>TA还没有喜欢的人。</c:otherwise></c:choose>
                </p>
            </div>
        </c:when>
        <c:otherwise>
...
<script type="text/javascript">
    ...
    function scrollPage() {
        ...
        if (sTop >= sNum && !scrollLock) {
            ...
            $.ajax({
                url: 'http://api.' + joyconfig.DOMAIN + '/joymeapp/gameclient/api/profile/likelist?uid=' + uid + '&pnum=' + (curPage + 1) + '&pcount=' + pageSize,
                type: 'GET',
