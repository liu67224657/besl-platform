 ...
 * @see Transactor
 */
public class PacketReceiverBase extends PoolThread {
	
	private static final Logger logger = LoggerFactory.getLogger(PacketReceiverBase.class);
	
    public boolean stopFlag = false;
    public DataInputStream inputStream;
    public PacketListener packetListener = null;
    public PacketReaderBase packetReader;
     ...
     * @see Connection
     */
    public PacketReceiverBase(Connection conn, PacketReaderBase pr) {
        connection = conn;
     ...
     */
    public void run() {
        logger.info("PacketReceiver.run(): Thread is starting");
        ...
        try {
            while (!stopFlag) {
                ...
                if (logger.isDebugEnabled()) {
                	logger.debug("PacketReceiver thread: packet read: " + rp);
                }
            ...
            logger.error("ReaderThread: Throwable: " + e.getMessage(), e);
            if (!(e instanceof IOException)) {
                logger.error("Error in receiving packet: " + rp, e);
            }
