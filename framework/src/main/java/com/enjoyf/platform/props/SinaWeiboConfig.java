...
public class SinaWeiboConfig {
    ...
    private SinaWeiboConfig() {
        init();
    }

    public static synchronized SinaWeiboConfig get() {
        if (instance == null) {
            instance = new SinaWeiboConfig();
        }
    ...
    private void init() {
        props = new FiveProps(HotdeployConfigFactory.get().getConfig(ThirdApiHotdeployConfig.class).getProviderPropsMap().get(LoginDomain.SINAWEIBO));
