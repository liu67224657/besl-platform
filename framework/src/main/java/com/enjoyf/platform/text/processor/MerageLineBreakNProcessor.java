 ...
 */
public class MerageLineBreakNProcessor implements TextProcessor {

    private String destStr;

    public MerageLineBreakNProcessor(String destStr) {
        this.destStr = destStr;
    }

    @Override
    public ResolveContent process(ResolveContent content) {
        String contentText=RegexUtil.replace(content.getContent(),HotdeployConfigFactory.get().getConfig(ContextHotdeployConfig.class).getLineBreakNFetchRegex(), destStr, -1);
