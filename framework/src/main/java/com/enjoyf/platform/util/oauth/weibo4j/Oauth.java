...
public class Oauth {
    ...
    public String parseSignedRequest(String signed_request) throws IOException,
        ...
        SecretKey key = new SecretKeySpec(SinaWeiboConfig.get().getClientSercret().getBytes(), "hmacSHA256");
        Mac m;
    ...
    public AccessToken getAccessTokenByCode(String code) throws WeiboException {
        return new AccessToken(Weibo.client.post(SinaWeiboConfig.get().getAccessTokenUri(),
                new PostParameter[]{
                        new PostParameter("client_id", SinaWeiboConfig.get().getClientID()),
                        new PostParameter("client_secret", SinaWeiboConfig.get().getClientSercret()),
                        new PostParameter("grant_type", "authorization_code"),
                        new PostParameter("code", code),
                        new PostParameter("redirect_uri", SinaWeiboConfig.get().getRedirectUri())}, false));
    }
    ...
    public String authorize(String response_type, String redirectUri) throws WeiboException {
        return SinaWeiboConfig.get().getAuthorizeUri().trim() + "?client_id="
                + SinaWeiboConfig.get().getClientID().trim() + "&redirect_uri="
                + redirectUri
